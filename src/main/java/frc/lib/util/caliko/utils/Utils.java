...
/**
 * Class : A series of static utility / helper methods to perform common operations. Version: 0.4
 * Date : 04/12/2015
 */
public final class Utils {
     ...
     *
     * <p>These colours are neither final nor 'constant', so they can be user modified at runtime if
     * desired.
     */
     ...
     *
     * <p>If you want a reproducable sequence of events, then set a seed value using
     * Utils.setRandomSeed(someValue).
     *
    ...
    /**
     * The maximum length in characters of any names which may be used for bones, chains or
     * structures.
     */
    ...
    /**
     * Set a fixed seed value - call this with any value before starting the inverse kinematics runs
     * to get a repeatable sequence of events.
     *
     ...
     *
     * <p>This means that, for example, a call to {@code randRange(-5.0f, 5.0f)} may return a value
     * between -5.0f up to a maximum of 4.999999f.
     *
     ...
     *
     * <p>This means that, for example, a call to {@code randRange(-5, 5)} will return a value
     * between -5 up to a maximum of 4.
     *
     * @param min The minimum value
     * @param max The maximum value
    ...
    public static FloatBuffer createFloatBuffer(int numFloats) {
        return ByteBuffer.allocateDirect(numFloats * Float.BYTES)
     ...
     *
     * <p>This allows you to have a repoducable sequence of pseudo-random numbers which are used by
     ...
     *
     * <p>If the direction unit vector has a magnitude of zero then an IllegalArgumentException is
     * thrown.
     *
     ...
     *
     * <p>If the provided bone length is not greater than zero then an IllegalArgumentException is
     * thrown.
     *
     ...
     *
     * <p>If the original range is approximately zero then the returned value is the average value
     * of the new range, that is: (newMin + newMax) / 2.0f
     *
    ...
    public static float convertRange(
            float origValue, float origMin, float origMax, float newMin, float newMax) {
    ...
    /**
     * Return a boolean indicating whether a float approximately equals another to within a given
     * tolerance.
     *
     * @param a The first value
     * @param b The second value
     * @param tolerance The difference within the <strong>a</strong> and <strong>b</strong> values
     *     must be within to be considered approximately equal.
     ...
     *
     * <p>Valid line widths are between 1.0f and 32.0f pixels inclusive.
     *
     * <p>Line widths outside this range will cause an IllegalArgumentException to be thrown.
     *
    ...
    public static void validateLineWidth(float lineWidth) {
        if (lineWidth < 1.0f || lineWidth > 32.0f) {
            throw new IllegalArgumentException(
                    "Line widths must be within the range 1.0f to 32.0f - but only 1.0f is"
