...
public class Vec3f implements Vectorf<Vec3f>, Serializable {
    ...
    // A Vec3 simply has three properties called x, y and z - these are public so we can access them
    // directly for speed
    public float x, y, z;
    ...
    public Vec3f() {
        /* x, y and z are initialised to zero by default as that's how primitives are initialised in Java*/
    }
    ...
    /**
     * Return whether the two provided vectors are perpendicular (to a dot-product tolerance of
     * 0.01f).
     *
    ...
    /**
     * Return whether the length of this Vec3f is approximately equal to a given value to within a
     * given tolerance.
     *
     ...
     * @return A boolean indicating whether the length of this vector is approximately the same as
     ...
     *
     * <p>Note: It is actually <em>this</em> vector which is negated and returned, not a copy /
     * clone.
     *
     ...
     * @param a The first vector.
     * @param b The second vector.
     * @param tolerance The value which each component of each vector must be within to be
     *     considered approximately equal.
     * @return Whether the two provided vector arguments are approximately equal (true) or not
     *     (false).
     */
     ...
     *
     * <p>If the provided vectors are normalised then this will be the same as the dot product.
     *
     ...
     * Return the scalar product of two vectors.
     *
     * <p>Normalised versions of the provided vectors are used in the dot product operation.
     *
     * @param v1 The first vector.
     * @param v2 The second vector.
     ...
     *
     * <p>The returned vector is not normalised.
     *
     * @param v1 The first vector.
     * @param v2 The second vector.
     ...
     *
     * <p>The Manhattan distance is an approximate distance between two points, but can be
     * calculated faster than the exact distance.
     *
     * <p>Further reading: http://en.wikipedia.org/wiki/floataxicab_geometry
     ...
     *
     * <p>Note: This vector itself is not modified - a new vector is created, each component is made
     * positive, and the new vector is returned.
     *
     ...
     *
     * <p>This is a very fast method of generating a perpendicular vector that works for any vector
     * which is 5 degrees or more from vertical 'up'.
     *
     * <p>The code in this method is adapted from:
     ...
     *
     * <p>The returned vector is normalised.
     *
     * <p>The code in this method is adapted from:
     * http://blog.selfshadow.com/2011/10/17/perp-vectors/
     *
     * <p>Further reading: Hughes, J. F., Muller, T., "Building an Orthonormal Basis from a Unit
     * Vector", Journal of Graphics Tools 4:4 (1999), 33-35.
     *
    ...
    // Further reading: Stark, M. M., "Efficient Construction of Perpendicular Vectors without
    // Branching", Journal of Graphics Tools 14:1 (2009), 55-61.
    ...
    public static Vec3f getUvBetween(Vec3f v1, Vec3f v2) {
        return new Vec3f(v2.minus(v1)).normalise();
    }

    /**
     * Calculate and return the angle between two vectors in radians.
     *
     * <p>The result will always be a positive value between zero and pi (3.14159f) radians.
     *
     * <p>This method does not modify the provided vectors, but does use normalised versions of them
     * in the calculations.
     *
     * @param v1 The first vector.
     * @param v2 The second vector.
     ...
     *
     * <p>The result will always be a positive value between [0..180) degrees.
     *
     * <p>This method does not modify the provided vectors, but does use normalised versions of them
     * in the calculations.
     *
     ...
     * @param otherVector The vector we will use to calculate the signed angle with respect to the
     *     reference vector.
     * @param normalVector The normal vector (i.e. vector perpendicular to) both the reference and
     *     'other' vectors.
     * @return The signed angle from the reference vector to the other vector in degrees.
     */
    public static float getSignedAngleBetweenDegs(
            Vec3f referenceVector, Vec3f otherVector, Vec3f normalVector) {
        float unsignedAngle = Vec3f.getAngleBetweenDegs(referenceVector, otherVector);
        float sign =
                Utils.sign(
                        Vec3f.dotProduct(
                                Vec3f.crossProduct(referenceVector, otherVector), normalVector));
     ...
     * <p>
     *
     * @param vecToLimit The vector which we will limit to a given angle with regard to the the
     *     baseline vector.
     * @param vecBaseline The vector which will be used as the baseline / frame-of-reference when
     *     rotating the vecToLimit.
     * @param angleLimitDegs The maximum angle which the vecToLimit may be rotated away from the
     *     vecBaseline, in degrees.
     * @return The rotated vecToLimit, which is constraint to a maximum of the angleLimitDegs
     *     argument.
     */
    public static Vec3f getAngleLimitedUnitVectorDegs(
            Vec3f vecToLimit, Vec3f vecBaseline, float angleLimitDegs) {
        ...
        if (angleBetweenVectorsDegs > angleLimitDegs) {
            // The axis which we need to rotate around is the one perpendicular to the two vectors -
            // so we're
            // rotating around the vector which is the cross-product of our two vectors.
            // Note: We do not have to worry about both vectors being the same or pointing in
            // opposite directions
            // because if they bones are the same direction they will not have an angle greater than
            // the angle limit,
            // and if they point opposite directions we will approach but not quite reach the
            // precise max angle
            // limit of 180.0f (I believe).
            Vec3f correctionAxis =
                    Vec3f.crossProduct(vecBaseline.normalised(), vecToLimit.normalised())
                            .normalise();

            // Our new vector is the baseline vector rotated by the max allowable angle about the
            // correction axis
            return Vec3f.rotateAboutAxisDegs(vecBaseline, angleLimitDegs, correctionAxis)
                    .normalised();
        } else // Angle not greater than limit? Just return a normalised version of the vecToLimit
        {
            // This may already BE normalised, but we have no way of knowing without calcing the
            // length, so best be safe and normalise.
            // TODO: If performance is an issue, then I could get the length, and if it's not
            // approx. 1.0f THEN normalise otherwise just return as is.
            return vecToLimit.normalised();
        }
    }

    /**
     * Return the global pitch of this vector about the global X-Axis. The returned value is within
     * the range -179.9f..180.0f degrees.
     *
    ...
    /**
     * Return the global yaw of this vector about the global Y-Axis. The returned value is within
     * the range -179.9f..180.0f degrees.
     *
    ...
    public static Vec3f rotateAboutAxisRads(Vec3f source, float angleRads, Vec3f rotationAxis) {
        ...
        // It's quicker to pre-calc these and reuse than calculate x * y, then y * x later (same
        // thing).
    ...
    /**
     * Return a vector which is the result of multiplying this vector by another vector. This vector
     * remains unchanged.
     *
     ...
     *
     * <p>This method does not perform any memory allocations - it merely adds 'other' to 'source'.
     *
     ...
     *
     * <p>This method does not perform any memory allocations - it merely subtracts 'other' from
     * 'source'.
     *
    ...
    /**
     * Return a vector which is the result of projecting this vector onto a plane described by the
     * provided surface normal.
     *
     * <p>Neither the vector on which this method is called or the provided plane normal vector are
     * modified.
     *
     * <p>If the plane surface normal has a magnitude of zero then an IllegalArgumentException is
     * thrown.
     *
     * @param planeNormal The normal that describes the plane onto which we will project this
     *     vector.
    ...
    public Vec3f projectOntoPlane(Vec3f planeNormal) {
        ...
        // Projection of vector b onto plane with normal n is defined as: b - ( b.n / ( |n| squared
        // )) * n
        ...
        /**
         * IMPORTANT: We have to be careful here - even code like the below (where dotProduct uses
         * normalised versions of 'this' and planeNormal is off by enough to make the IK solutions
         * oscillate:
         *
         * <p>return this.minus( planeNormal.times( Vec3f.dotProduct(this, planeNormal) )
         * ).normalised();
         */

        // Note: For non-normalised plane vectors we can use:
        // float planeNormalLength = planeNormal.length();
        // return b.minus( n.times( Vec3f.dotProduct(b, n) / (planeNormalLength * planeNormalLength)
        // ).normalised();
    }
     ...
     *
     * <p>If the opposite direction is required then the argument order can be swapped or the the
     * result can simply be negated.
     *
    ...
    /**
     * Randomise the components of this vector to be random values between the provided half-open
     * range as described by the minimum and maximum value arguments.
     *
     * @param min The minimum value for any given component (inclusive).
     * @param max The maximum value for any given component (exclusive, i.e. a max of 5.0f will be
     *     assigned values up to 4.9999f or such).
     */
