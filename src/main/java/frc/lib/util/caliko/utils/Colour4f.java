 ...
 *
 * <p>The r, g, b and a properties of the class are publicly accessible for direct access, but
 * constructors and the 'set' method clamp the colour components to the valid range 0.0f to 1.0f.
 * Setting colour component values outside of this range may result in undefined behaviour.
 *
 * @author Al Lansley
...
public class Colour4f implements Serializable {
     ...
     * Default constructor.
     *
     * <p>The constructed Colour4f has it's RGBA components set to default values of 1.0f, which
     * equates to white at full opacity.
     */
    public Colour4f() {
        r = g = b = a = 1.0f;
    }
     ...
     * Array Constructor.
     *
     * <p>If the array size is not 4, then an IllegalArgument exception is thrown.
     *
    ...
    public Colour4f(float[] sourceValues) {
        if (sourceValues.length == 4) {
            ...
            throw new IllegalArgumentException(
                    "Colour source array size must be precisely 4 elements.");
        }
    }
     ...
     * Float Constructor.
     *
     * <p>The valid range of each component is 0.0f to 1.0f inclusive, any values outside of this
     ...
     *
     * <p>Source values are clamped to the range 0.0f..1.0f.
     *
     ...
     *
     * <p>Any values outside the range 0.0f..1.0f are clamped to the nearest valid value.
     *
     * <p>
     *
    ...
    /**
     * Add to the RGB components of this colour by the given amounts and return this modified colour
     * for chaining.
     *
     * <p>When adding, colour values are clamped to a maximum value of 1.0f.
     *
    ...
    /**
     * Subtract from the RGB components of this colour by the given amounts and return this modified
     * colour for chaining.
     *
     * <p>When subtracting, colour values are clamped to a minimum value of 1.0f.
     *
     * @param red The red component to add to this colour.
     * @param green The green component to add to this colour.
     * @param blue The blue component to add to this colour.
     ...
     *
     * <p>Resulting colour components are clamped to the range 0.0f..1.0f.
     *
    ...
    public static Colour4f randomOpaqueColour() {
        return new Colour4f(
                Utils.random.nextFloat(), Utils.random.nextFloat(), Utils.random.nextFloat(), 1.0f);
    }

    // ---------- Private Methods ----------

    private static float clamp(final float componentValue) {
        if (componentValue > MAX_COMPONENT_VALUE) {
            return MAX_COMPONENT_VALUE;
        } else if (componentValue < MIN_COMPONENT_VALUE) {
            return MIN_COMPONENT_VALUE;
        } else {
