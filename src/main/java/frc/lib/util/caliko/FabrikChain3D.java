...
import frc.lib.util.caliko.FabrikChain3D.BaseboneConstraintType3D;
import frc.lib.util.caliko.FabrikJoint3D.JointType;
import frc.lib.util.caliko.utils.Colour4f;
import frc.lib.util.caliko.utils.Mat3f;
import frc.lib.util.caliko.utils.Utils;
import frc.lib.util.caliko.utils.Vec3f;
...
/**
 * Class to represent a 3D Inverse Kinematics (IK) chain that can be solved for a given target using
 * the FABRIK algorithm.
 *
 * <p>A FabrikChain3D consists primarily of a list of connected {@link
 * frc.lib.util.caliko.FabrikBone3D} objects, and a number of parameters which keep track of
 * settings related to how we go about solving the IK chain.
 *
 * @author Al Lansley
 * @version 0.5.3 - 19/06/2019
 */
public class FabrikChain3D
        implements FabrikChain<FabrikBone3D, Vec3f, FabrikJoint3D, BaseboneConstraintType3D>,
                Serializable {
    ...
    public enum BaseboneConstraintType3D implements BaseboneConstraintType {
        ...
        LOCAL_ROTOR, // Rotor constraint in the coordinate space of (i.e. relative to) the direction
        // of the connected bone
        GLOBAL_HINGE, // World-space hinge constraint
        LOCAL_HINGE // Hinge constraint in the coordinate space of (i.e. relative to) the direction
        // of the connected bone
    }
    ...
    /**
     * The core of a FabrikChain3D is a list of FabrikBone3D objects. It is this chain that we
     * attempt to solve for a specified target location via the solveForTarget method(s).
     */
     ...
     *
     * <p>Although entirely optional, it may be used to uniquely identify a specific FabrikChain3D
     * in an an array/list/map or such of FabrikChain3D objects.
     *
     * @see #setName
     * @see #getName
     */
    ...
    /**
     * The distance threshold we must meet in order to consider this FabrikChain3D to be
     * successfully solved for distance.
     *
     * <p>When we solve a chain so that the distance between the end effector and target is less
     * than or equal to the distance threshold, then we consider the chain to be solved and will
     * dynamically abort any further attempts to solve the chain.
     *
     * <p>The default solve distance threshold is <strong>1.0f</strong>.
     *
     * <p>The minimum valid distance threshold is 0.0f, however a slightly higher value should be
     * used to avoid forcing the IK chain solve process to run repeatedly when an
     * <strong>acceptable</strong> (but not necessarily <em>perfect</em>) solution is found. Setting
     * a very low solve distance threshold may result in significantly increased processor usage and
     * hence increased processing time to solve a given IK chain.
     *
     * <p>Although this property is the main criteria used to establish whether or not we have
     * solved a given IK chain, it works in combination with the {@link #mMaxIterationAttempts} and
     * {@link mMinIterationChange} fields to improve the performance of the algorithm in situations
     * where we may not be able to solve a given IK chain. Such situations may arise when bones in
     * the chain are highly constrained, or when the target is further away than the length of a
     * chain which has a fixed base location.
     *
     * <p>See {@link #setSolveDistanceThreshold(float) } See {@link #mMaxIterationAttempts } See
     * {@link #mMinIterationChange }
     */
    private float mSolveDistanceThreshold = 1.0f;

    /**
     * maxIterationAttempts (int) Specifies the maximum number of attempts that will be performed in
     * order to solve the IK chain. If we have not solved the chain to within the solve distance
     * threshold after this many attempts then we accept the best solution we have best on solve
     * distance to target.
     *
     * <p>The default is 20 iteration attempts.
     */
    private int mMaxIterationAttempts = 20;

    /**
     * minIterationChange (float) Specifies the minimum distance improvement which must be made per
     * solve attempt in order for us to believe it worthwhile to continue making attempts to solve
     * the IK chain. If this iteration change is not exceeded then we abort any further solve
     * attempts and accept the best solution we have based on solve distance to target.
     *
     * <p>The default is 0.01f.
     */
    private float mMinIterationChange = 0.01f;

    /**
     * chainLength (float) The chainLength is the combined length of all bones in this FabrikChain3D
     * object.
     *
     * <p>When a FabrikBone3D is added or removed from the chain using the addBone,
     * addConsecutiveBone or removeBone methods, then the chainLength is updated to reflect this.
     *
     * <p>See {@link #addBone(FabrikBone3D)} See {@link #addConsecutiveBone(FabrikBone3D)} See
     * {@link #removeBone(int)}
     */
     ...
     *
     * <p>By default, FabrikChain3D objects are created with a fixed base location, that is the
     * start joint of the first bone in the chain is not moved during the solving process. A user
     * may still move this base location by calling setFixedBaseMode(boolean) and the FABRIK
     * algorithm will then honour this new location as the 'fixed' base location.
     *
     * <p>The default is Vec3f(0.f, 0.0f).
     *
     * <p>See {@link #setFixedBaseMode(boolean)}
     */
     ...
     *
     * <p>By default, the location of the start joint of the first bone added to the IK chain is
     * considered fixed. This 'anchors' the base of the chain in place. Optionally, a user may
     * toggle this behaviour by calling {@link #setFixedBaseMode(boolean)} to enable or disable
     * locking the basebone to a fixed starting location.
     *
     * <p>See {@link #setFixedBaseMode(boolean)}
     */
    ...
    /**
     * Each chain has a BaseboneConstraintType3D - this may be either: - NONE, // No constraint -
     * basebone may rotate freely - GLOBAL_ROTOR, // World-space rotor (i.e. ball joint) constraint
     * - LOCAL_ROTOR, // Rotor constraint which is relative to the coordinate space of the connected
     * bone - GLOBAL_HINGE, // World-space hinge constraint, or - LOCAL_HINGE // Hinge constraint
     * which is relative to the coordinate space of the connected bone
     */
     ...
     *
     * <p>To ensure correct operation, the provided Vec3f is normalised inside the {@link
     * #setBaseboneConstraintUV(Vec3f)} method. Passing a Vec3f with a magnitude of zero will result
     * in the constraint not being set.
     */
    private Vec3f mBaseboneConstraintUV = new Vec3f();

    /**
     * mBaseboneRelativeConstraintUV The basebone direction constraint in the coordinate space of
     * the bone in another chain that this chain is connected to.
     */
    private Vec3f mBaseboneRelativeConstraintUV = new Vec3f();

    /**
     * mBaseboneRelativeReferenceConstraintUV The basebone reference constraint in the coordinate
     * space of the bone in another chain that this chain is connected to.
     */
     ...
     *
     * <p>The target location can be updated via the {@link #solveForTarget(Vec3f)} or {@link
     * #solveForTarget(float, float, float)} methods, which in turn will call the solveIK(Vec3f)
     * method to attempt to solve the IK chain, resulting in an updated chain configuration.
     *
     * <p>The default is Vec3f(Float.MAX_VALUE, Float.MAX_VALUE, Float.MAX_VALUE)
     */
    private Vec3f mLastTargetLocation =
            new Vec3f(Float.MAX_VALUE, Float.MAX_VALUE, Float.MAX_VALUE);

    /**
     * The width in pixels of the line used to draw constraints for this chain.
     *
     * <p>The valid range is 1.0f to 32.0f inclusive.
     *
     * <p>The default is 2.0f pixels.
     */
     ...
     *
     * <p>We keep track of the previous base location in order to use it to determine if the current
     * base location and previous base location are the same, i.e. has the base location moved
     * between the last run to this run? If the base location has moved, then we MUST solve the IK
     * chain for this new base location - even if the target location has remained the same between
     * runs.
     *
     * <p>The default is Vec3f(Float.MAX_VALUE, Float.MAX_VALUE, Float.MAX_VALUE).
     *
    ...
    /**
     * mCurrentSolveDistance The current distance between the end effector and the target location
     * for this IK chain.
     *
     * <p>The current solve distance is updated when an attempt is made to solve the IK chain as
     * triggered by a call to the {@link #solveForTarget(Vec3f)} or (@link #solveForTarget(float,
     * float, float) methods.
     */
     ...
     *
     * <p>If the value is -1 then it's not connected to another bone or chain.
     *
     * <p>The default is -1.
     */
    ...
    /**
     * The zero-indexed number of the bone that this chain is connected to, if it's connected to
     * another chain at all.
     *
     * <p>If the value is -1 then it's not connected to another bone or chain.
     *
     * <p>The default is -1.
     ...
     *
     * <p>Embedded target locations allow structures to be solved for multiple targets (one per
     * chain in the structure) rather than all chains being solved for the same target. To use
     * embedded targets, the mUseEmbeddedTargets flag must be true (which is not the default) - this
     * flag can be set via a call to setEmbeddedTargetMode(true).
     *
     * <p>See (@link #setEmbeddedTargetMode(boolean) }
     */
    private Vec3f mEmbeddedTarget = new Vec3f();

    /**
     * mUseEmbeddedTarget Whether or not to use the mEmbeddedTarget location when solving this
     * chain.
     *
     * <p>This flag may be toggled by calling the setEmbeddedTargetMode(true) on the chain.
     *
     ...
     *
     * <p>This chain's {@link mChainLength} property is updated to take into account the length of
     * the new bone added to the chain.
     *
     * <p>In addition, if the bone being added is the very first bone, then this chain's {@link
     * mFixedBaseLocation} property is set from the start joint location of the bone.
     *
    ...
    /**
     * Add a bone to the end of this IK chain given the direction unit vector and length of the new
     * bone to add.
     *
     * <p>The bone added does not have any rotational constraints enforced, and will be drawn with a
     * default colour of white at full opacity.
     *
     * <p>This method can only be used when the IK chain contains a basebone, as without it we do
     * not have a start location for this bone (i.e. the end location of the previous bone).
     *
     * <p>If this method is executed on a chain which does not contain a basebone then a {@link
     * RuntimeException} is thrown.
     *
     * <p>If this method is provided with a direction unit vector of zero, or a bone length of zero
     * then then an {@link IllegalArgumentException} is thrown.
     *
    ...
    /**
     * Add a consecutive bone to the end of this IK chain given the direction unit vector and length
     * of the new bone to add.
     *
     * <p>The bone added does not have any rotational constraints enforced, and will be drawn with a
     * default colour of white at full opacity.
     *
    ...
    public void addConsecutiveBone(Vec3f directionUV, float length, Colour4f colour) {
        // Validate the direction unit vector - throws an IllegalArgumentException if it has a
        // magnitude of zero
        Utils.validateDirectionUV(directionUV);

        // Validate the length of the bone - throws an IllegalArgumentException if it is not a
        // positive value
        Utils.validateLength(length);

        // If we have at least one bone already in the chain...
        if (!mChain.isEmpty()) {
            // Get the end location of the last bone, which will be used as the start location of
            // the new bone
        ...
        } else // Attempting to add a relative bone when there is no basebone for it to be relative
        // to?
        {
            throw new RuntimeException(
                    "You cannot add the basebone as a consecutive bone as it does not provide a"
                            + " start location. Use the addBone() method instead.");
        }
    }
    ...
    public void addConsecutiveBone(FabrikBone3D bone) {
        ...
        if (!this.mChain.isEmpty()) {
            ...
            // Add a bone to the end of this IK chain
            addBone(bone);
        } else // Attempting to add a relative bone when there is no base bone for it to be relative
        // to?
        {
            throw new RuntimeException(
                    "You cannot add the base bone to a chain using this method as it does not"
                            + " provide a start location.");
        }
    }

    /**
     * Add a consecutive hinge constrained bone to the end of this chain. The bone may rotate freely
     * about the hinge axis.
     *
     * <p>The bone will be drawn with a default colour of white.
     *
     * <p>This method can only be used when the IK chain contains a basebone, as without it we do
     * not have a start location for this bone (i.e. the end location of the previous bone).
     *
     * <p>If this method is executed on a chain which does not contain a basebone then a
     * RuntimeException is thrown. If this method is provided with a direction unit vector of zero,
     * then an IllegalArgumentException is thrown. If the joint type requested is not
     * JointType.LOCAL_HINGE or JointType.GLOBAL_HINGE then an IllegalArgumentException is thrown.
     * If this method is provided with a hinge rotation axis unit vector of zero, then an
     * IllegalArgumentException is thrown.
     *
     * @param directionUV The initial direction of the new bone.
     * @param length The length of the new bone.
     * @param jointType The type of hinge joint to be used - either JointType.LOCAL or
     *     JointType.GLOBAL.
     * @param hingeRotationAxis The axis about which the hinge joint freely rotates.
     */
    public void addConsecutiveFreelyRotatingHingedBone(
            Vec3f directionUV, float length, JointType jointType, Vec3f hingeRotationAxis) {
        // Because we aren't constraining this bone to a reference axis within the hinge rotation
        // axis we don't care about the hinge constraint
        // reference axis (7th param) so we'll just generate an axis perpendicular to the hinge
        // rotation axis and use that.
        addConsecutiveHingedBone(
                ...
                180.0f,
                180.0f,
                Vec3f.genPerpendicularVectorQuick(hingeRotationAxis),
                new Colour4f());
    }
     ...
     *
     * <p>The hinge type may be a global hinge where the rotation axis is specified in world-space,
     * or a local hinge, where the rotation axis is relative to the previous bone in the chain.
     *
     * <p>If this method is executed on a chain which does not contain a basebone then a
     * RuntimeException is thrown. If this method is provided with bone direction or hinge
     * constraint axis of zero then an IllegalArgumentException is thrown. If the joint type
     * requested is not LOCAL_HINGE or GLOBAL_HINGE then an IllegalArgumentException is thrown.
     *
     * @param directionUV The initial direction of the new bone.
     * @param length The length of the new bone.
     ...
     * @param hingeReferenceAxis The axis about which any clockwise/anticlockwise rotation
     *     constraints are enforced.
     * @param colour The colour to draw the bone.
     */
    public void addConsecutiveHingedBone(
            Vec3f directionUV,
            float length,
            JointType jointType,
            Vec3f hingeRotationAxis,
        ...
        // Get the end location of the last bone, which will be used as the start location of the
        // new bone
        Vec3f prevBoneEnd = mChain.get(mChain.size() - 1).getEndLocation();
        ...
        switch (jointType) {
            case GLOBAL_HINGE:
                joint.setAsGlobalHinge(
                        hingeRotationAxis, clockwiseDegs, anticlockwiseDegs, hingeReferenceAxis);
                break;
            case LOCAL_HINGE:
            ...
            default:
                throw new IllegalArgumentException(
                        "Hinge joint types may be only JointType.GLOBAL_HINGE or"
     ...
     * @param clockwiseDegs The clockwise constraint angle in degrees.
     * @param anticlockwiseDegs The anticlockwise constraint angle in degrees.
     * @param hingeConstraintReferenceAxis The reference axis about which any
     *     clockwise/anticlockwise rotation constraints are enforced.
     */
    public void addConsecutiveHingedBone(
            Vec3f directionUV,
            float length,
     ...
     * <p>If this method is executed on a chain which does not contain a basebone then a
     * RuntimeException is thrown. If this method is provided with bone direction or hinge
     * constraint axis of zero then an IllegalArgumentException is thrown.
     *
     ...
     * @param colour The colour to draw the bone.
     */
    public void addConsecutiveRotorConstrainedBone(
            Vec3f boneDirectionUV, float boneLength, float constraintAngleDegs, Colour4f colour) {
        ...
        // Create the bone starting at the end of the previous bone, set its direction, constraint
        // angle and colour
        // then add it to the chain. Note: The default joint type of a new FabrikBone3D is
        // JointType.BALL.
        FabrikBone3D bone =
                new FabrikBone3D(
                        mChain.get(mChain.size() - 1).getEndLocation(),
     ...
     * Add a consecutive rotor (i.e. ball joint) constrained bone to the end of this IK chain.
     *
     * <p>The bone will be drawn in white at full opacity by default. This method can only be used
     * when the IK chain contains a basebone, as without it we do not have a start location for this
     * bone (i.e. the end location of the previous bone).
     *
     * <p>If this method is executed on a chain which does not contain a basebone then a
     * RuntimeException is thrown. If this method is provided with bone direction or hinge
     * constraint axis of zero then an IllegalArgumentException is thrown. If the joint type
     ...
     */
    public void addConsecutiveRotorConstrainedBone(
            Vec3f boneDirectionUV, float boneLength, float constraintAngleDegs) {
        addConsecutiveRotorConstrainedBone(
                boneDirectionUV, boneLength, constraintAngleDegs, new Colour4f());
    }

    /**
     * Return the basebone relative unit vector of this chain.
     *
     * <p>This direction is updated by the FabrikStructure3D when this chain is connected to another
     * chain. There is no other possible way of doing it as we have no knowledge of other chains,
     * but the structure does, allowing us to calculate this relative constraint UV.
     *
     * @return The basebone relative constraint UV as updated (on solve) by the structure containing
     *     this chain.
     */
    @Override
     ...
     *
     * <p>Valid values are 1.0f to 32.0f inclusive, although the OpenGL standard specifies that only
     * line widths of 1.0f are guaranteed to work. Values outside of this range will result in an
     ...
     *
     * <p>If the basebone is not constrained then a RuntimeException is thrown. If you wish to check
     * whether the basebone of this IK chain is constrained you may use the {@link
     * #getBaseboneConstraintType()} method.
     *
    ...
    public Vec3f getBaseboneConstraintUV() {
        if (mBaseboneConstraintType != BaseboneConstraintType3D.NONE) {
            ...
            throw new RuntimeException(
                    "Cannot return the basebone constraint when the basebone constraint type is"
                            + " NONE.");
        }
    }
     ...
     *
     * <p>Regardless of how many bones are contained in the chain, the base location is always the
     * start location of the first bone in the chain.
     *
     * <p>This method does not return the mBaseLocation property of this chain because the start
     * location of the basebone may be more up-to-date due to a moving 'fixed' location.
     *
    ...
    /**
     * Return the List%lt;FabrikBone3D%gt; which comprises the actual IK chain of this FabrikChain3D
     * object.
     *
     * @return The List%lt;FabrikBone3D%gt; which comprises the actual IK chain of this
     *     FabrikChain3D object.
     */
    @Override
     ...
     *
     * <p>This method does not dynamically re-calculate the length of the chain - it merely returns
     * the previously calculated chain length, which gets updated each time a bone is added or
     * removed from the chain. However, as the chain length is updated whenever necessary this
     * should be fine.
     *
     * <p>If you need a calculated-on-the-fly value for the chain length, then use the
     * getLiveChainLength() method.
     *
     ...
     *
     * <p>Returns -1 (default) if this chain is not connected to another chain.
     *
     * @return The zero-indexed number of the bone we are connected to in the chain we are connected
     *     to.
     */
    @Override
     ...
     *
     * <p>Regardless of how many bones are contained in the chain, the end effector is always the
     * end location of the final bone in the chain.
     *
     ...
     *
     * <p>Embedded target mode may be enabled or disabled using setEmbeddededTargetMode(boolean).
     *
     * @return whether or not this chain uses an embedded target.
     */
    @Override
     ...
     *
     * <p>The target location and the effector location are not necessarily at the same location
     * unless the chain has been solved for distance, and even then they are still likely to be
     * <i>similar</i> rather than <b>identical</b> values.
     *
     ...
     *
     * <p>Typically, the getChainLength() can be called which returns the length of the chain as
     * updated / recalculated when a bone is added or removed from the chain (which is significantly
     * faster as it doesn't require recalculation), but sometimes it may be useful to get the
     * definitive most up-to-date chain length so you can check if operations being performed have
     * altered the chain length - hence this method.
     *
     ...
     *
     * <p>This chain's {@link mChainLength} property is updated to take into account the new chain
     * length.
     *
     * <p>If the bone number to be removed does not exist in the chain then an
     * IllegalArgumentException is thrown.
     *
    ...
    public void removeBone(int boneNumber) {
        ...
        if (boneNumber < mChain.size()) {
            ...
            throw new IllegalArgumentException(
                    "Bone "
                            + boneNumber
                            + " does not exist to be removed from the chain. Bones are zero"
                            + " indexed.");
        }
    }

    /**
     * Set the relative basebone constraint UV - this direction should be relative to the coordinate
     * space of the basebone.
     *
     * <p>This function is deliberately made package-private as it should not be used by the end
     * user - instead, the FabrikStructure3D.solveForTarget() method will update this
     * mBaseboneRelativeConstraintUV property FOR USE BY this chain as required.
     *
     * <p>The reason for this is that this chain on its own cannot calculate the relative constraint
     * direction, because it relies on direction of the connected / 'host' bone in the chain that
     * this chain is connected to - only we have no knowledge of that other chain! But, the
     * FabrikStructure3D DOES have knowledge of that other chain, and is hence able to calculate and
     * update this relative basebone constraint direction for us.
     */
    ...
    /**
     * Set the relative basebone reference constraint UV - this direction should be relative to the
     * coordinate space of the basebone.
     *
     * <p>This function is deliberately made package-private as it should not be used by the end
     * user - instead, the FabrikStructure3D.solveForTarget() method will update this
     * mBaseboneRelativeConstraintUV property FOR USE BY this chain as required.
     *
     * <p>This property is required when we have a LOCAL_HINGE basebone constraint with reference
     * axes - we must maintain the hinge's own rotation and reference axes, and then the
     * FabrikStructure3D.solveForTarget() method updates the mBaseboneRelativeConstraintUV and
     * mBaseboneRelativeReferenceConstraintUV as required.
     */
     ...
     *
     * <p>Depending on whether the constraint type is GLOBAL_ROTOR or LOCAL_ROTOR the constraint
     * will be applied about global space or about the local coordinate system of a bone in another
     * chain that this chain is attached to.
     *
     * <p>The angle provided should be between the range of 0.0f (completely constrained) to 180.0f
     * (completely free to rotate). Values outside of this range will be clamped to the relevant
     * minimum or maximum.
     *
     * <p>If this chain does not contain a basebone then a RuntimeException is thrown. If the
     * constraint axis is a zero vector or the rotor type is not GLOBAL_ROTOR or LOCAL_ROTOR then
     ...
     */
    public void setRotorBaseboneConstraint(
            BaseboneConstraintType3D rotorType, Vec3f constraintAxis, float angleDegs) {
        // Sanity checking
        if (mChain.isEmpty()) {
            throw new RuntimeException(
                    "Chain must contain a basebone before we can specify the basebone constraint"
                            + " type.");
        }
        ...
        if (!(rotorType == BaseboneConstraintType3D.GLOBAL_ROTOR
     ...
     *
     * <p>If the number of bones in this chain is zero (i.e. it does not contain a basebone) then a
     * RuntimeException is thrown. If the hinge rotation or reference axes are zero vectors then an
     * IllegalArgumentException is thrown. If the hinge reference axis does not lie in the plane of
     * the hinge rotation axis (that is, they are not perpendicular) then an
     * IllegalArgumentException is thrown.
     *
     ...
     * @param cwConstraintDegs The clockwise constraint angle about the hinge reference axis in
     *     degrees.
     * @param acwConstraintDegs The clockwise constraint angle about the hinge reference axis in
     *     degrees.
     * @param hingeReferenceAxis The axis (perpendicular to the hinge rotation axis) about which the
     *     constraint angles apply.
     */
    public void setHingeBaseboneConstraint(
            BaseboneConstraintType3D hingeType,
            Vec3f hingeRotationAxis,
        ...
        if (!(Vec3f.perpendicular(hingeRotationAxis, hingeReferenceAxis))) {
            throw new IllegalArgumentException(
                    "The hinge reference axis must be in the plane of the hinge rotation axis, that"
                            + " is, they must be perpendicular.");
        }
     ...
     *
     * <p>The clockwise and anticlockwise constraint angles are automatically set to 180 degrees and
     * the hinge reference axis is generated to be any vector perpendicular to the hinge rotation
     * axis.
     *
     * <p>If the number of bones in this chain is zero (i.e. it does not contain a basebone) then a
     * RuntimeException is thrown. If the hinge rotation axis are zero vectors then an
     * IllegalArgumentException is thrown.
     *
     ...
     */
    public void setLocalHingedBasebone(
            Vec3f hingeRotationAxis, float cwDegs, float acwDegs, Vec3f hingeReferenceAxis) {
        setHingeBaseboneConstraint(
                BaseboneConstraintType3D.LOCAL_HINGE,
                hingeRotationAxis,
                cwDegs,
     ...
     * @param cwDegs The clockwise constraint angle in degrees.
     * @param acwDegs The anticlockwise constraint angle in degrees.
     * @param hingeReferenceAxis The global / world-space reference axis about which the hinge is
     *     constrained.
     */
    public void setGlobalHingedBasebone(
            Vec3f hingeRotationAxis, float cwDegs, float acwDegs, Vec3f hingeReferenceAxis) {
        setHingeBaseboneConstraint(
                BaseboneConstraintType3D.GLOBAL_HINGE,
                hingeRotationAxis,
     ...
     *
     * <p>This method constrains the <strong>basebone</strong> (<em>only</em>) to a global direction
     * unit vector.
     *
     * <p>Attempting to set the basebone constraint when the bone has a basebone constraint type of
     * NONE or providing a constraint vector of zero will result will result in an
     * IllegalArgumentException being thrown.
     *
     * @param constraintUV The direction unit vector to constrain the basebone to.
     * @see frc.lib.util.caliko.caliko.FabrikJoint3D#setBallJointConstraintDegs(float angleDegs)
     * @see frc.lib.util.caliko.caliko.FabrikJoint3D#setHingeJointClockwiseConstraintDegs(float)
     * @see frc.lib.util.caliko.caliko.FabrikJoint3D#setHingeJointAnticlockwiseConstraintDegs(float)
     */
    @Override
    public void setBaseboneConstraintUV(Vec3f constraintUV) {
        if (mBaseboneConstraintType == BaseboneConstraintType3D.NONE) {
            throw new IllegalArgumentException(
                    "Specify the basebone constraint type with setBaseboneConstraintTypeCannot"
                            + " specify a basebone constraint when the current constraint type is"
                            + " BaseboneConstraint.NONE.");
        }
     ...
     *
     * <p>The assignment is made by reference so that this base location and the location where we
     * attach to the other chain are the same Vec3f object.
     *
     * <p>Note: If this chain is attached to another chain then this 'fixed' base location will be
     * updated as and when the connection point in the chain we are attached to moves.
     *
     ...
     *
     * <p>In order to connect this chain to another chain, both chains must exist within the same
     * structure.
     *
     * <p>If the structure does not contain the specified chain or bone then an
     * IllegalArgumentException is thrown.
     *
     * @param structure The structure which contains the chain which contains the bone to connect
     *     to.
     ...
     *
     * <p>If the basebone is 'fixed' in place, then its start location cannot move. The bone is
     * still allowed to rotate, with or without constraints.
     *
     * <p>Specifying a non-fixed base location while this chain is connected to another chain will
     * result in a RuntimeException being thrown.
     *
     * <p>Fixing the basebone's start location in place and constraining to a global absolute
     * direction are mutually exclusive. Disabling fixed base mode while the chain's constraint type
     * is BaseboneConstraintType3D.GLOBAL_ABSOLUTE will result in a RuntimeException being thrown. *
     *
     * @param value Whether or not to fix the basebone start location in place.
     */
    @Override
    public void setFixedBaseMode(boolean value) {
        // Enforce that a chain connected to another chain stays in fixed base mode (i.e. it moves
        // with the chain it's connected to instead of independently)
        if (!value && mConnectedChainNumber != -1) {
            throw new RuntimeException(
                    "This chain is connected to another chain so must remain in fixed base mode.");
        }

        // We cannot have a freely moving base location AND constrain the basebone to an absolute
        // direction
        if (mBaseboneConstraintType == BaseboneConstraintType3D.GLOBAL_ROTOR && !value) {
            throw new RuntimeException(
                    "Cannot set a non-fixed base mode when the chain's constraint type is"
                            + " BaseboneConstraintType3D.GLOBAL_ABSOLUTE_ROTOR.");
        }
     ...
     *
     * <p>The FABRIK algorithm may require more than a single pass in order to solve a given IK
     * chain for an acceptable distance threshold. If we reach this iteration limit then we stop
     * attempting to solve the IK chain. Further details on this topic are provided in the {@link
     * #mMaxIterationAttempts} documentation.
     *
     * <p>If a maxIterations value of less than 1 is provided then an IllegalArgumentException is
    ...
    /**
     * Set the minimum iteration change before we dynamically abort any further attempts to solve
     * this IK chain.
     *
     * <p>If the latest solution found has changed by less than this amount then we consider the
     * progress being made to be not worth the computational effort and dynamically abort any
     * further attempt to solve the chain for the current target to minimise CPU usage.
     *
     * <p>If a minIterationChange value of less than zero is specified then an
     * IllegalArgumentException is thrown.
     *
     * @param minIterationChange The minimum change in solve distance from one iteration to the
     *     next.
     */
    @Override
     ...
     *
     * <p>If a solve distance value of less than zero is specified then an IllegalArgumentException
     * is thrown.
     *
     * @param solveDistance The distance between the end effector of this IK chain and target within
     *     which we will accept the solution.
     */
    @Override
     ...
     *
     * <p>The embedded target location can be updated by calling updateEmbeddedTarget(Vec3f).
     *
     * @return The distance between the end effector and the chain's embedded target location for
     *     our best solution.
     */
    @Override
    public float solveForEmbeddedTarget() {
        if (mUseEmbeddedTarget) {
            ...
            throw new RuntimeException(
                    "This chain does not have embedded targets enabled - enable with"
                            + " setEmbeddedTargetMode(true).");
        }
    }
     ...
     *
     * <p>The end result of running this method is that the IK chain configuration is updated.
     *
     * <p>To minimuse CPU usage, this method dynamically aborts if: - The solve distance (i.e.
     * distance between the end effector and the target) is below the {@link
     * mSolveDistanceThreshold}, - A solution incrementally improves on the previous solution by
     * less than the {@link mMinIterationChange}, or - The number of attempts to solve the IK chain
     * exceeds the {@link mMaxIterationAttempts}.
     *
     ...
     * @return The resulting distance between the end effector and the new target location after
     *     solving the IK chain.
     */
     ...
     * @return float The resulting distance between the end effector and the new target location
     *     after solving the IK chain.
     */
    @Override
    public float solveForTarget(Vec3f newTarget) {
        ...
        for (int loop = 0; loop < mMaxIterationAttempts; ++loop) {
            ...
            // Did we solve it for distance? If so, update our best distance and best solution, and
            // also
            // update our last pass solve distance. Note: We will ALWAYS beat our last solve
            // distance on the first run.
            if (solveDistance < bestSolveDistance) {
                bestSolveDistance = solveDistance;
                bestSolution = this.cloneIkChain();

                // If we are happy that this solution meets our distance requirements then we can
                // exit the loop now
            ...
            {
                // Did we grind to a halt? If so break out of loop to set the best distance and
     ...
     *
     * <p>If this chain does not contain any bones then a RuntimeException is thrown.
     *
     * @return The best solve distance found between the end-effector of this chain and the provided
     *     target.
     */
    private float solveIK(Vec3f target) {
        ...
        // Loop over all bones in the chain, from the end effector (numBones-1) back to the basebone
        // (0)
        for (int loop = mChain.size() - 1; loop >= 0; --loop) {
            ...
            if (loop != mChain.size() - 1) {
                ...
                if (thisBoneJointType == JointType.BALL) {
                    ...
                    float angleBetweenDegs =
                            Vec3f.getAngleBetweenDegs(
                                    outerBoneOuterToInnerUV, thisBoneOuterToInnerUV);
                    ...
                    if (angleBetweenDegs > constraintAngleDegs) {
                        thisBoneOuterToInnerUV =
                                Vec3f.getAngleLimitedUnitVectorDegs(
                                        thisBoneOuterToInnerUV,
                    ...
                    thisBoneOuterToInnerUV =
                            thisBoneOuterToInnerUV.projectOntoPlane(
                                    thisBoneJoint.getHingeRotationAxis());

                    // NOTE: Constraining about the hinge reference axis on this forward pass leads
                    // to poor solutions... so we won't.
                } else if (thisBoneJointType == JointType.LOCAL_HINGE) {
                    // Not a basebone? Then construct a rotation matrix based on the previous bones
                    // inner-to-to-inner direction...
                    Mat3f m;
                    Vec3f relativeHingeRotationAxis;
                    if (loop > 0) {
                        m = Mat3f.createRotationMatrix(mChain.get(loop - 1).getDirectionUV());
                        relativeHingeRotationAxis =
                                m.times(thisBoneJoint.getHingeRotationAxis()).normalise();
                    ...
                    // ...and transform the hinge rotation axis into the previous bones frame of
                    // reference.
                    // Vec3f

                    // Project this bone's outer-to-inner direction onto the plane described by the
                    // relative hinge rotation axis
                    // Note: The returned vector is normalised.
                    thisBoneOuterToInnerUV =
                            thisBoneOuterToInnerUV.projectOntoPlane(relativeHingeRotationAxis);

                    // NOTE: Constraining about the hinge reference axis on this forward pass leads
                    // to poor solutions... so we won't.
                }

                // At this stage we have a outer-to-inner unit vector for this bone which is within
                // our constraints,
                // so we can set the new inner joint location to be the end joint location of this
                // bone plus the
                // outer-to-inner direction unit vector multiplied by the length of the bone.
                Vec3f newStartLocation =
                        thisBone.getEndLocation()
            ...
            } else // If we ARE working on the end effector bone...
            {
                ...
                // Get the UV between the target / end-location (which are now the same) and the
                // start location of this bone
                Vec3f thisBoneOuterToInnerUV = thisBone.getDirectionUV().negated();
                ...
                switch (thisBoneJointType) {
                    case BALL:
                        ...
                        break;
                    case GLOBAL_HINGE:
                        // Global hinges get constrained to the hinge rotation axis, but not the
                        // reference axis within the hinge plane
                        thisBoneOuterToInnerUV =
                                thisBoneOuterToInnerUV.projectOntoPlane(
                                        thisBoneJoint.getHingeRotationAxis());
                        break;
                    case LOCAL_HINGE:
                        // Local hinges get constrained to the hinge rotation axis, but not the
                        // reference axis within the hinge plane

                        // Construct a rotation matrix based on the previous bones inner-to-to-inner
                        // direction...
                ...
                // Calculate the new start joint location as the end joint location plus the
                // outer-to-inner direction UV
                ...
                // ...and set the end joint location of the bone further in to also be at the new
                // start location (if there IS a bone
        ...
        for (int loop = 0; loop < mChain.size(); ++loop) {
            ...
            if (loop != 0) {
                // Get the inner-to-outer direction of this bone as well as the previous bone to use
                // as a baseline
                ...
                if (jointType == JointType.BALL) {
                    float angleBetweenDegs =
                            Vec3f.getAngleBetweenDegs(
                                    prevBoneInnerToOuterUV, thisBoneInnerToOuterUV);
                    float constraintAngleDegs = thisBoneJoint.getBallJointConstraintDegs();

                    // Keep this bone direction constrained within the rotor about the previous bone
                    // direction
                    if (angleBetweenDegs > constraintAngleDegs) {
                    ...
                    if (!(Utils.approximatelyEquals(
                        ...
                        // Get the signed angle (about the hinge rotation axis) between the hinge
                        // reference axis and the hinge-rotation aligned bone UV
                        // Note: ACW rotation is positive, CW rotation is negative.
                        float signedAngleDegs =
                                Vec3f.getSignedAngleBetweenDegs(
                                        hingeReferenceAxis,
                                        thisBoneInnerToOuterUV,
                                        hingeRotationAxis);

                        // Make our bone inner-to-outer UV the hinge reference axis rotated by its
                        // maximum clockwise or anticlockwise rotation as required
                    ...
                    // Transform the hinge rotation axis to be relative to the previous bone in the
                    // chain
                    Vec3f hingeRotationAxis = thisBoneJoint.getHingeRotationAxis();
                    ...
                    // Project this bone direction onto the plane described by the hinge rotation
                    // axis
                    // Note: The returned vector is normalised.
                    thisBoneInnerToOuterUV =
                            thisBoneInnerToOuterUV.projectOntoPlane(relativeHingeRotationAxis);
                    ...
                    if (!(Utils.approximatelyEquals(
                        ...
                        Vec3f relativeHingeReferenceAxis =
                                m.times(thisBoneJoint.getHingeReferenceAxis()).normalise();
                ...
                // bone plus the
                // outer-to-inner direction unit vector multiplied by the length of the bone.
                Vec3f newEndLocation =
                        thisBone.getStartLocation()
                                .plus(thisBoneInnerToOuterUV.times(thisBoneLength));

                // Set the new start joint location for this bone
                thisBone.setEndLocation(newEndLocation);

                // If we are not working on the end effector bone, then we set the start joint
                // location of the next bone in
                // the chain (i.e. the bone closer to the target) to be the new end joint location
                // of this bone.
                if (loop < mChain.size() - 1) {
            ...
            {
                // If the base location is fixed then snap the start location of the basebone back
                // to the fixed base...
                if (mFixedBaseMode) {
                ...
                if (mBaseboneConstraintType == BaseboneConstraintType3D.NONE) {
                    ...
                    // then set the start location of the next bone to be the end location of this
                    // bone
                    Vec3f newEndLocation =
                            thisBone.getStartLocation()
                ...
                } else // ...otherwise we must constrain it to the basebone constraint unit vector
                {
                    if (mBaseboneConstraintType == BaseboneConstraintType3D.GLOBAL_ROTOR) {
                        ...
                        float angleBetweenDegs =
                                Vec3f.getAngleBetweenDegs(
                                        mBaseboneConstraintUV, thisBoneInnerToOuterUV);
                        ...
                        // Also, set the start location of the next bone to be the end location of
                        // this bone
                        if (mChain.size() > 1) {
                            mChain.get(1).setStartLocation(newEndLocation);
                        }
                    } else if (mBaseboneConstraintType == BaseboneConstraintType3D.LOCAL_ROTOR) {
                        // Note: The mBaseboneRelativeConstraintUV is updated in the
                        // FabrikStructure3D.solveForTarget()
                        // method BEFORE this FabrikChain3D.solveForTarget() method is called. We no
                        // knowledge of the
                        // direction of the bone we're connected to in another chain and so cannot
                        // calculate this
                        // relative basebone constraint direction on our own, but the
                        // FabrikStructure3D does it for
                        // us so we are now free to use it here.

                        // Get the inner-to-outer direction of this bone
                        Vec3f thisBoneInnerToOuterUV = thisBone.getDirectionUV();

                        // Constrain about the relative basebone constraint unit vector as
                        // neccessary
                        float angleBetweenDegs =
                                Vec3f.getAngleBetweenDegs(
                                        mBaseboneRelativeConstraintUV, thisBoneInnerToOuterUV);
                        float constraintAngleDegs = thisBone.getBallJointConstraintDegs();
                        if (angleBetweenDegs > constraintAngleDegs) {
                            thisBoneInnerToOuterUV =
                                    Vec3f.getAngleLimitedUnitVectorDegs(
                        ...
                        float cwConstraintDegs =
                                -thisJoint
                                        .getHingeClockwiseConstraintDegs(); // Clockwise rotation is
                        // negative!
                        float acwConstraintDegs = thisJoint.getHingeAnticlockwiseConstraintDegs();

                        // Get the inner-to-outer direction of this bone and project it onto the
                        // global hinge rotation axis
                        Vec3f thisBoneInnerToOuterUV =
                                thisBone.getDirectionUV().projectOntoPlane(hingeRotationAxis);

                        // If we have a global hinge which is not freely rotating then we must
                        // constrain about the reference axis
                        if (!(Utils.approximatelyEquals(
                                        cwConstraintDegs,
                                        -FabrikJoint3D.MAX_CONSTRAINT_ANGLE_DEGS,
                                        0.01f)
                                && Utils.approximatelyEquals(
                                        ...
                                        FabrikJoint3D.MAX_CONSTRAINT_ANGLE_DEGS,
                                        0.01f))) {
                            // Grab the hinge reference axis and calculate the current signed angle
                            // between it and our bone direction (about the hinge
                            // rotation axis). Note: ACW rotation is positive, CW rotation is
                            // negative.
                        ...
                        FabrikJoint3D thisJoint = thisBone.getJoint();
                        Vec3f hingeRotationAxis =
                                mBaseboneRelativeConstraintUV; // Basebone relative constraint is
                        // our hinge rotation axis!
                        float cwConstraintDegs =
                                -thisJoint
                                        .getHingeClockwiseConstraintDegs(); // Clockwise rotation is
                        // negative!
                        ...
                        // If we have a local hinge which is not freely rotating then we must
                        // constrain about the reference axis
                        if (!(Utils.approximatelyEquals(
                                        cwConstraintDegs,
                                        -FabrikJoint3D.MAX_CONSTRAINT_ANGLE_DEGS,
        ...
        // DEBUG - check the live chain length and the originally calculated chain length are the
        // same
        /*
        ...
        */
        ...
        // Finally, calculate and return the distance between the current effector location and the
        // target.
    ...
    public void updateChainLength() {
        ...
        // Loop over all the bones in the chain, adding the length of each bone to the mChainLength
        // property
        for (FabrikBone3D aBone : this.mChain) {
     ...
     *
     * <p>The internal mEmbeddedTarget object is updated with the location of the provided
     * parameter. If the chain is not in useEmbeddedTarget mode then a RuntimeException is thrown.
    ...
    /**
     * Clone and return the IK Chain of this FabrikChain3D, that is, the list of FabrikBone3D
     * objects.
     *
    ...
    private List<FabrikBone3D> cloneIkChain() {
        ...
        for (FabrikBone3D aBone : this.mChain) {
            // Use the copy constructor to create a new FabrikBone3D with the values set from the
            // source FabrikBone3D.
    ...
    public int hashCode() {
        ...
        result =
                prime * result
                        + ((mBaseboneConstraintType == null)
                                ? 0
                                : mBaseboneConstraintType.hashCode());
        result =
                prime * result
                        + ((mBaseboneConstraintUV == null) ? 0 : mBaseboneConstraintUV.hashCode());
        result =
                prime * result
                        + ((mBaseboneRelativeConstraintUV == null)
                                ? 0
                                : mBaseboneRelativeConstraintUV.hashCode());
        result =
                prime * result
                        + ((mBaseboneRelativeReferenceConstraintUV == null)
                                ? 0
                                : mBaseboneRelativeReferenceConstraintUV.hashCode());
        ...
        result =
                prime * result + ((mFixedBaseLocation == null) ? 0 : mFixedBaseLocation.hashCode());
        result = prime * result + (mFixedBaseMode ? 1231 : 1237);
        result = prime * result + ((mLastBaseLocation == null) ? 0 : mLastBaseLocation.hashCode());
        result =
                prime * result
                        + ((mLastTargetLocation == null) ? 0 : mLastTargetLocation.hashCode());
    ...
    public boolean equals(Object obj) {
        ...
        } else if (!mBaseboneRelativeReferenceConstraintUV.equals(
        ...
        if (Float.floatToIntBits(mConstraintLineWidth)
                != Float.floatToIntBits(other.mConstraintLineWidth)) {
            return false;
        }
        if (Float.floatToIntBits(mCurrentSolveDistance)
                != Float.floatToIntBits(other.mCurrentSolveDistance)) {
            return false;
        }
        ...
        if (Float.floatToIntBits(mMinIterationChange)
                != Float.floatToIntBits(other.mMinIterationChange)) {
            return false;
        }
        if (mName == null) {
            if (other.mName != null) {
        ...
        if (Float.floatToIntBits(mSolveDistanceThreshold)
                != Float.floatToIntBits(other.mSolveDistanceThreshold)) {
            return false;
        }
