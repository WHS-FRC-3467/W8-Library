...
import frc.lib.util.caliko.FabrikChain2D.BaseboneConstraintType2D;
import frc.lib.util.caliko.FabrikJoint2D.ConstraintCoordinateSystem;
...
/**
 * Class to represent a 2D Inverse Kinematics (IK) chain that can be solved for a given target using
...
public class FabrikChain2D
        implements FabrikChain<FabrikBone2D, Vec2f, FabrikJoint2D, BaseboneConstraintType2D>,
                Serializable {
     ...
     *
     * <p>Basebone constraints constrain the basebone (i.e. the first bone in a chain) and may be of
     * the following types:
     *
     * <ul>
     *   <li>NONE - No constraint.
     *   <li>GLOBAL_ABSOLUTE - Constrained about a world-space direction.
     *   <li>LOCAL_RELATIVE - Constrained about the direction of the connected bone.
     *   <li>LOCAL_ABSOLUTE - Constrained about a direction with relative to the direction of the
     *       connected bone.
     * </ul>
     *
    ...
    /**
     * The core of a FabrikChain2D is a list of FabrikBone2D objects, where each bone contains a
     * start and end location, and a joint that stores any rotational constraints.
     */
     ...
     *
     * <p>Although entirely optional, it may be used to uniquely identify a specific FabrikChain2D
     * in an an array/vector/map or such.
     *
     * <p>See {@link frc.lib.util.caliko.FabrikChain#setName(String)} See {@link
     * frc.lib.util.caliko.FabrikChain#getName()}
     */
    private String mName;

    /**
     * The distance threshold we must meet in order to consider this FabrikChain2D to be
     * successfully solved for distance.
     *
     * <p>When we solve a chain so that the distance between the end effector and target is less
     * than or equal to the distance threshold, then we consider the chain to be successfully solved
     * and will dynamically abort any further attempts to solve the chain.
     *
     ...
     * <strong>acceptable</strong> (but not necessarily <em>perfect</em>) solution is found.
     *
     ...
     *
     * <p>The default value is 1.0f.
     *
     * <p>See {@link #setSolveDistanceThreshold(float)}
     */
    private float mSolveDistanceThreshold = 1.0f;

    /**
     * Specifies the maximum number of attempts that will be performed in order to solve this
     * FabrikChain2D.
     *
     * <p>As FABRIK is an iterative algorithm, after a single solve attempt we may not be within the
     * solve distance threshold for the chain, so must take the current chain configuration and
     * perform another solve attempt. With a reasonable value for the solve distance threshold (i.e.
     * the distance which we must solve the chain for in order to consider it solved), it is common
     * to be able to solve a given IK chain within a small number of attempts.
     *
     * <p>However, some IK chains cannot be solved, whether this is due to joint constraints, or (in
     * the case of an IK chain with a fixed base location) where the distance between the base
     * location and the target is greater than the length of the chain itself. In these cases, we
     * may abort any further attempts to solve the chain after this number of solve attempts. This
     * decreases the execution time of the algorithm, as there is no point in repeatedly attempting
     * to solve an IK chain which cannot be solved.
     *
     * <p>Further, we may find that we are making unacceptably low progress towards solving the
     * chain, then will also abort solving the chain based on the {@link mMinIterationChange}
     * threshold.
     *
     * <p>The default value is 15.
     *
     * <p>See {@link #setMaxIterationAttempts(int)}
     */
    private int mMaxIterationAttempts = 15;

    /**
     * Specifies the minimum change in the solve distance which must be made from one solve attempt
     * to the next in order for us to believe it worthwhile to continue making attempts to solve the
     * IK chain.
     *
     * <p>As FABRIK is an iterative algorithm, after a single solve attempt we may not be within the
     * solve distance threshold for the chain, so must take the current chain configuration and
     ...
     * the case of an IK chain with a fixed base location) where the distance between the base
     * location and the target is greater than the length of the chain itself. In these cases, we
     * abort attempting to solve the chain if we are making very little progress towards solving the
     * chain between solve attempts. This decreases the execution time of the algorithm, as there is
     * no point in repeatedly attempting to solve an IK chain which cannot be solved.
     *
     * <p>This property must be greater than or equal to zero.
     *
     * <p>The default value is 0.01f.
     *
     * <p>See {@link #setMinIterationChange(float)}
     */
     ...
     *
     * <p>When a FabrikBone2D is added or removed from the chain using the
     * addBone/addConsecutiveBone or removeBone methods, then the chainLength is updated to reflect
     * this.
     */
     ...
     *
     * <p>By default, FabrikChain2D objects are created with a fixed base location, that is, the
     * start joint of the first bone in the chain is immovable. As such, the start joint location of
     * the first bone in the chain is stored in this property so that the base of the chain can be
     * snapped back to this location during the FABRIK solve process.
     *
     * <p>Base locations do not <em>have</em> to be fixed - this property can be modified to via the
     * setFixedBaseLocation method.
     *
     ...
     * considered fixed. This 'anchors' the base of the chain in place. Optionally, a user may call
     * the setFixedBaseMode method to disable fixing the base bone start location in place.
     *
     * <p>The default value is true (i.e. the basebone of the chain is fxied in place).
     *
     * <p>See {@link #setFixedBaseMode(boolean)}
     */
     ...
     *
     * <p>The default value is BaseboneConstraintType2D.NONE.
     */
    private BaseboneConstraintType2D mBaseboneConstraintType = BaseboneConstraintType2D.NONE;

    /**
     * If this chain is connected to a bone in another chain, then this property controls whether it
     * should connect to the start end location of the host bone.
     *
     * <p>The default value is BoneConnectionPoint.END.
     *
     * <p>See {@link #setBoneConnectionPoint(BoneConnectionPoint)} See {@link
     * FabrikStructure2D#connectChain(FabrikChain2D, int, int, BoneConnectionPoint)}
     */
    private BoneConnectionPoint mBoneConnectionPoint = BoneConnectionPoint.END;

    /**
     * The direction around which we should constrain the base bone, as provided to the
     * setBaseboneConstraintUV method.
     *
     * <p>See {@link #setBaseboneConstraintUV(Vec2f)}
     */
    private Vec2f mBaseboneConstraintUV = new Vec2f();

    /**
     * The (internally used) constraint unit vector when this chain is connected to another chain in
     * either LOCAL_ABSOLUTE or LOCAL_RELATIVE modes.
     *
     * <p>This property cannot be accessed by users and is updated in the
     * FabrikStructure2D.solveForTarget() method.
     */
     ...
     *
     * <p>The last target location is used during the solve attempt and is specified as a property
     * of the chain to avoid memory allocation at runtime.
     */
     ...
     *
     * <p>The default value is Vec2f(Float.MAX_VALUE, Float.MAX_VALUE).
     */
     ...
     * flag can be set via a call to setEmbeddedTarget(true).
     *
     ...
     * chain.
     *
     * <p>This flag may be toggled by calling the setEmbeddedTargetMode(boolean) method on the
     * chain.
     *
     ...
     * for this IK chain.
     *
     * <p>The current solve distance is updated when an attempt is made to solve IK chain as
     * triggered by a call to the {@link #solveForTarget(Vec2f)} or (@link #solveForTarget(float,
     * float) methods.
     */
     ...
     *
     * <p>* If this value is -1 then this chain is not connected to another chain.
     *
     ...
     *
     * <p>If the value is -1 then it's not connected to another bone in another chain.
     *
     * <p>The default value is -1.
     */
     ...
     */
    public FabrikChain2D(String name) {
        mName = name;
    }
     ...
     * mBaseLocation} property is set from the start joint location of the bone.
     *
     ...
     *
     * <p>The constraint angles are relative to the coordinate system of the previous bone in the
     * chain.
     *
     * <p>This method can only be used when the IK chain contains a base bone, as without it we do
     * not have a start location for this bone (i.e. the end location of the previous bone).
     *
     * <p>If this method is called on a chain which does not contain a base bone then a {@link
     * RuntimeException} is thrown.
     *
     * <p>Specifying a direction unit vector of zero will result in an IllegalArgumentException
     * being thrown.
     *
     ...
     */
    public void addConsecutiveConstrainedBone(
            Vec2f directionUV, float length, float clockwiseDegs, float anticlockwiseDegs) {
        addConsecutiveConstrainedBone(
                directionUV, length, clockwiseDegs, anticlockwiseDegs, new Colour4f());
    }
     ...
     *
     * <p>If there are no bones in the chain when this method is called then a RuntimeException is
     * thrown.
     *
    ...
    public Vec2f getBaseLocation() {
        if (!this.mChain.isEmpty()) {
            ...
            throw new RuntimeException(
                    "Cannot get base location as there are zero bones in the chain.");
        }
    }
     ...
     *
     * <p>The base bone is always bone 0, each additional bone increases the number by 1.
     *
     ...
     *
     * <p>The connection point will be either BoneConnectionPoint2D.START or
     * BoneConnectionPoint2D.END.
     *
     * @return The bone connection point of this chain.
     */
     ...
     * the previously calculated chain length, which gets updated each time a bone is added to the
     * chain.
     *
    ...
    /**
     * Return the number of the chain in the structure containing this FabrikChain2D that this chain
     * is connected to.
     *
    ...
    public Vec2f getEffectorLocation() {
        if (!this.mChain.isEmpty()) {
            ...
            throw new RuntimeException(
                    "Cannot get effector location as there are zero bones in the chain.");
        }
    }
     ...
     *
     * <p>The last target location and the current target location may not be the same.
     *
     ...
     *
     * <p>Bones may be added to the chain via the addBone or addConsecutiveBone methods.
     *
     ...
     *
     * <p>Bone numbers start at 0, so if there were 3 bones in a chain they would be at locations 0,
     * 1 and 2.
     *
     * <p>This chain's {@link mChainLength} property is updated to take into account the new chain
     * length.
     *
     * <p>If this chain does not contain a bone at the specified location then an
    ...
    public void removeBone(int boneNumber) {
        ...
        if (boneNumber < this.mChain.size()) {
            ...
            throw new IllegalArgumentException(
                    "Bone " + boneNumber + " does not exist in this chain.");
        }
    }
    ...
    public void setBaseboneConstraintType(BaseboneConstraintType2D type) {
        mBaseboneConstraintType = type;
    }

    /**
     * Set the constraint unit vector for the basebone of this chain..
     *
     * <p>The bone will be constrained about the clockwise and anticlockwise constraint angles with
     * regard to this direction methods on the FabrikBone2D to be constrained.
     *
     ...
     * @see frc.lib.util.caliko.FabrikJoint2D#setClockwiseConstraintDegs
     * @see frc.lib.util.caliko.FabrikJoint2D#setAnticlockwiseConstraintDegs
     */
    @Override
     ...
     * @see #mBaseLocation
     */
    public void setBaseLocation(Vec2f baseLocation) {
     ...
     *
     * <p>When this chain is connected to another chain, it may either connect to the bone in the
     * other chain at the start (BoneConnectionPoint.START) or end (BoneConnectionPoint.END) of the
     * bone.
     *
     * <p>This property is held per-chain rather than per-bone for efficiency.
     *
    ...
    /**
     * Set which number bone this chain is connected to in another chain in a FabrikStructure2D
     * object.
     *
     ...
     *
     * <p>If the base bone is 'fixed' in place, then its start location cannot move unless it is
     * attached to another chain (which moves). The bone is still allowed to rotate, with or without
     * constraints.
     *
     ...
     *
     * <p>Fixing the base bone's start location in place and constraining to a global absolute
     * direction are mutually exclusive. Disabling fixed base mode while the chain's constraint type
     * is BaseBoneConstraintType2D.GLOBAL_ABSOLUTE will result in a RuntimeException being thrown.
     *
    ...
    public void setFixedBaseMode(boolean value) {
        ...
        // We cannot have a freely moving base location AND constrain the base bone - so if we have
        // both
        ...
        if (mBaseboneConstraintType == BaseboneConstraintType2D.GLOBAL_ABSOLUTE && !value) {
            throw new RuntimeException(
                    "Cannot set a non-fixed base mode when the chain's constraint type is"
                            + " BaseBoneConstraintType2D.GLOBAL_ABSOLUTE.");
        }
     ...
     * attempting to solve the IK chain.
     *
     * <p>Specifying a maxIterations value of less than 1 will result in an IllegalArgumentException
     * is thrown.
     *
     ...
     * this IK chain.
     *
     * <p>If the current solve distance changes by less than this amount between solve attempt then
     * we consider the solve process to have stalled and dynamically abort any further attempts to
     * solve the chain to minimise CPU usage.
     *
     * <p>If a minIterationChange value of less than zero is provided then an
     * IllegalArgumentException is thrown.
     *
     ...
     *
     * <p>If a solve distance of less than zero is provided then an IllegalArgumentException is
     * thrown.
     *
     ...
     *
     * <p>The FABRIK algorithm itself can be found in the following paper: Aristidou, A., & Lasenby,
     * J. (2011). FABRIK: a fast, iterative solver for the inverse kinematics problem. Graphical
     * Models, 73(5), 243-260.
     *
     * <p>The end result of running this method is that the IK chain configuration is updated (i.e.
     * joint locations for each bone in the chain are modified in order to solve the chain for the
     * provided target location).
     *
     * <p>
     *
     * @param target The target location to solve the IK chain for.
     * @return float The distance between the end effector and the given target location after
     *     attempting to solve the IK chain.
     */
    private float solveIK(Vec2f target) {
        // ---------- Step 1 of 2 - Forward pass from end-effector to base -----------

        // Loop over all bones in the chain, from the end effector (numBones-1) back to the base
        // bone (0)
        for (int loop = this.mChain.size() - 1; loop >= 0; --loop) {
            ...
            if (loop != this.mChain.size() - 1) {
                ...
                // Note: On the forward pass we constrain to the limits imposed by joint of the
                // outer bone.
                float clockwiseConstraintDegs = outerBone.getJoint().getClockwiseConstraintDegs();
                float antiClockwiseConstraintDegs =
                        outerBone.getJoint().getAnticlockwiseConstraintDegs();

                Vec2f constrainedUV;
                if (mChain.get(loop).getJointConstraintCoordinateSystem()
                        == FabrikJoint2D.ConstraintCoordinateSystem.LOCAL) {
                    constrainedUV =
                            Vec2f.getConstrainedUV(
                ...
                } else // Constraint is in global coordinate system
                {
                    constrainedUV =
                            Vec2f.getConstrainedUV(
                                    thisBoneOuterToInnerUV,
                                    thisBone.getGlobalConstraintUV().negated(),
                                    clockwiseConstraintDegs,
                                    antiClockwiseConstraintDegs);
                }
                ...
                Vec2f newStartLocation =
                        thisBone.getEndLocation().plus(constrainedUV.times(boneLength));
            ...
            } else // If we are working on the end effector bone ...
            {
                ...
                if (loop > 0) {
                    ...
                    // Note: On the forward pass we constrain to the limits imposed by the first
                    // joint of the inner bone.
                    float clockwiseConstraintDegs =
                            thisBone.getJoint().getClockwiseConstraintDegs();
                    float antiClockwiseConstraintDegs =
                            thisBone.getJoint().getAnticlockwiseConstraintDegs();

                    // If this bone is locally constrained...
                    if (thisBone.getJoint().getConstraintCoordinateSystem()
                            == ConstraintCoordinateSystem.LOCAL) {
                ...
                } else // There is only one bone in the chain, and the bone is both the basebone and
                // the end-effector bone.
                {
                    // Don't constraint (nothing to constraint against) if constraint is in local
                    // coordinate system
                    if (thisBone.getJointConstraintCoordinateSystem()
                            == FabrikJoint2D.ConstraintCoordinateSystem.LOCAL) {
        ...
        for (int loop = 0; loop < this.mChain.size(); ++loop) {
            ...
            if (loop != 0) {
                ...
                // Note: On the backward pass we constrain to the limits imposed by the first joint
                // of this bone.
                ...
                // At this stage we have an inner-to-outer unit vector for this bone which is within
                // our constraints,
                // so we can set the new end location to be the start location of this bone plus the
                // constrained
                // inner-to-outer direction unit vector multiplied by the length of this bone.
                Vec2f newEndLocation =
                        thisBone.getStartLocation().plus(constrainedUV.times(boneLength));
            ...
            {
                // If the base location is fixed then snap the start location of the base bone back
                // to the fixed base
                if (mFixedBaseMode) {
                    mChain.get(0).setStartLocation(mBaseLocation);
                } else // If the base location is not fixed...
                {
                    // ...then set the new base bone start location to be its the end location minus
                    // the
                    // bone direction multiplied by the length of the bone (i.e. projected
                    // backwards).
                    ...
                    Vec2f newBoneZeroStartLocation =
                            boneZeroEndLocation.minus(boneZeroUV.times(boneLength));
                ...
                if (mBaseboneConstraintType == BaseboneConstraintType2D.NONE) {
                    ...
                    // Calculate the new end location as the start location plus the direction times
                    // the length of the bone
                    Vec2f newEndLocation =
                            thisBone.getStartLocation()
                                    .plus(thisBoneInnerToOuterUV.times(boneLength));

                    // Set the new end joint location
                    mChain.get(0).setEndLocation(newEndLocation);

                    // Also, set the start location of the next bone to be the end location of this
                    // bone
                    if (mChain.size() > 1) {
                ...
                {
                    // Note: The mBaseBoneConstraintUV is either fixed, or it may be dynamically
                    // updated from
                    ...
                    // Get the constrained direction unit vector between the base bone and the base
                    // bone constraint unit vector
                    // Note: On the backward pass we constrain to the limits imposed by the first
                    // joint of this bone.
                    float clockwiseConstraintDegs =
                            thisBone.getJoint().getClockwiseConstraintDegs();
                    float antiClockwiseConstraintDegs =
                    ...
                    // LOCAL_ABSOLUTE? (i.e. local-space directional constraint) - then we must
                    // constraint about the relative basebone constraint UV...
                    Vec2f constrainedUV;
                    if (mBaseboneConstraintType == BaseboneConstraintType2D.LOCAL_ABSOLUTE) {
                        ...
                        //						System.out.println("Relative constraint UV = " +
                        // mBaseboneRelativeConstraintUV);
                    ...
                    // At this stage we have an inner-to-outer unit vector for this bone which is
                    // within our constraints,
                    // so we can set the new end location to be the start location of this bone plus
                    // the constrained
                    // inner-to-outer direction unit vector multiplied by the length of the bone.
                    Vec2f newEndLocation =
                            mChain.get(loop)
                                    .getStartLocation()
                                    .plus(constrainedUV.times(boneLength));

                    // Set the new end joint location for this bone
                    mChain.get(loop).setEndLocation(newEndLocation);

                    // If we are not working on the end bone, then we set the start joint location
                    // of
                    // the next bone in the chain (i.e. the bone closer to the end effector) to be
                    // the
        ...
        // ...and calculate and return the distance between the current effector location and the
        // target.
    ...
    /**
     * Clone and return the IK Chain of this FabrikChain2D, that is, the Vector of FabrikBone2D
     * objects.
     *
     * <p>This does not clone the entire FabrikChain2D object, only its IK chain.
     *
    ...
    private List<FabrikBone2D> cloneChainVector() {
        ...
        // Create a new Vector of FabrikBone2D objects large enough to contain all the bones in this
        // chain
        ...
        for (int loop = 0; loop < numBones; ++loop) {
            // ...use the FabrikBone2D copy constructor to create a new FabrikBone2D with the
            // properties as set from the source bone
     ...
     *
     * <p>The embedded target location can be updated by calling updateEmbeddedTarget(Vec2f).
     *
     ...
     *
     * <p>We will iteratively attempt up to solve the chain up to a maximum of
     * mMaxIterationAttempts.
     *
     * <p>This method may return early if any of the following conditions are met:
     *
     * <ul>
     *   <li>We've already solved for this target location,
     *   <li>We successfully solve for distance, or
     *   <li>We grind to a halt (i.e. low iteration change compared to previous solution).
     * </ul>
     *
     * <p>This method simply constructs a Vec2f from the provided arguments and calls the {@link
     * Vec2f} version of the solveForTarget method. By making this available, the user does not need
     * to use our custom {@link Vec2f} class in their application.
     *
     ...
     * </ul>
     *
     * The solution may NOT change we cannot improve upon the calculated distance between the new
     * target location and the existing solution.
     *
    ...
    public float solveForTarget(Vec2f newTarget) {
        ...
        if (mLastTargetLocation.approximatelyEquals(newTarget, 0.001f)
                && mLastBaseLocation.approximatelyEquals(mBaseLocation, 0.001f)) {
        ...
        // If the base location of a chain hasn't moved then we may opt to keep the current solution
        // if our
        // best new solution is worse...
        if (mLastBaseLocation.approximatelyEquals(mBaseLocation, 0.001f)) {
            startingDistance =
                    Vec2f.distanceBetween(
                            mChain.get(mChain.size() - 1).getEndLocation(), newTarget);
            startingSolution = this.cloneChainVector();
        } else // Base has changed? Then we have little choice but to recalc the solution and take
        // that new solution.
        {
        ...
        // We'll keep track of our best solve distance, starting it at a huge value which will be
        // beaten on first attempt
        ...
        // Did we get a solution that's better than the starting solution's to the new target
        // location?
        ...
        // Update our last base and target locations so we know whether we need to solve for this
        // start/end configuration next time
     ...
     */
    public void setBaseboneRelativeConstraintUV(Vec2f constraintUV) {
        mBaseboneRelativeConstraintUV.set(constraintUV);
    }
    ...
    public int hashCode() {
        ...
        result =
                prime * result
                        + ((mBoneConnectionPoint == null) ? 0 : mBoneConnectionPoint.hashCode());
