...
/**
 * A FabrikStructure3D contains one or more FabrikChain3D objects, which we can solve using the
 * FABRIK (Forward And Backward Reaching Inverse Kinematics) algorithm for specified target
 * locations.
 *
 * <p>The FabrikStructure3D class is merely a convenient holder for a list of FabrikChain3D objects
 * which allows multiple chains to have their target location updated, as well as solving and
 * drawing the multiple chains attached to the FabrikStructure3D object using one method call per
 * structure.
 *
 * <p>If you do not intend on attaching multiple FabrikChain3D objects into a complex structure, for
 * example one with multiple effectors, then you may be better served by creating individual
 * FabrikChain3D objects and using those objects directly.
 *
 * @author Al Lansley
...
public class FabrikStructure3D implements FabrikStructure<FabrikChain3D, Vec3f>, Serializable {
     ...
     *
     * <p>Each FabrikChain3D in the mChains vector is independent of all others, but shares the same
     * target location as any/all other chains which exist in this structure.
     */
     ...
     * Naming constructor.
     *
     * <p>Name lengths are truncated to a maximum of 100 characters, if necessary.
     *
    ...
    public void solveForTarget(Vec3f newTargetLocation) {
        ...
        for (int loop = 0; loop < numChains; ++loop) {
            // Get this chain, and get the number of the chain in this structure it's connected to
            // (if any)
            ...
            } else // ...however, if this chain IS connected to another chain...
            {
                ...
                // Now that we've clamped the base location of this chain to the start or end point
                // of the bone in the chain we are connected to, it's
                ...
                switch (constraintType) {
                    // None or global basebone constraints? Nothing to do, because these will be
                    // handled in FabrikChain3D.solveIK() as we do not
                    // need information from another chain to handle them.
                    case NONE: // Nothing to do because there's no basebone constraint
                    case GLOBAL_ROTOR: // Nothing to do because the basebone constraint is not
                    // relative to bones in other chains in this structure
                    case GLOBAL_HINGE: // Nothing to do because the basebone constraint is not
                        // relative to bones in other chains in this structure
                        break;

                    // If we have a local rotor or hinge constraint then we must calculate the
                    // relative basebone constraint before calling updateTarget
                    case LOCAL_ROTOR:
                    case LOCAL_HINGE:
                        {
                            // Get the direction of the bone this chain is connected to and create a
                            // rotation matrix from it.
                            Mat3f connectionBoneMatrix =
                                    Mat3f.createRotationMatrix(hostBone.getDirectionUV());

                            // We'll then get the basebone constraint UV and multiply it by the
                            // rotation matrix of the connected bone
                            // to make the basebone constraint UV relative to the direction of bone
                            // it's connected to.
                            Vec3f relativeBaseboneConstraintUV =
                                    connectionBoneMatrix
                                            .times(thisChain.getBaseboneConstraintUV())
                                            .normalised();
                        ...
                        // No need for a default - constraint types are enums and we've covered them
                        // all.
                }

                // NOTE: If the base bone constraint type is NONE then we don't do anything with the
                // base bone constraint of the connected chain.
     ...
     *
     * <p>In effect, the chain is added to the mChains list of FabrikChain3D objects, and the
     * mNumChains property is incremented.
     *
     * <p>Adding a chain using this method adds the chain to the structure, but does not connect it
     * to any existing chain in the structure. If you wish to connect a chain, use one of the
     * connectChain methods instead.
     *
     * <p>All chains in a structure share the same target, and all chains in the structure can be
     * solved for the target location via a single call to updateTarget.
     *
     ...
     *
     * <p>In effect, the chain is removed from the mChains list of FabrikChain3D objects, and the
     * mNumChains property is decremented.
     *
     * @param chainIndex The index of the chain to remove from the mChains list of FabrikChain3D
     *     objects.
     */
     ...
     *
     * <p>Both chains and bones are are zero indexed.
     *
     * <p>If the existingChainNumber or existingBoneNumber specified to connect to does not exist in
     * this structure then an IllegalArgumentExeception is thrown.
     *
     ...
     * @param existingBoneNumber The index of the bone to connect the new chain to within the
     *     existing chain.
     */
    @Override
    public void connectChain(
            FabrikChain3D newChain, int existingChainNumber, int existingBoneNumber) {
        ...
        // Make a copy of the provided chain so any changes made to the original do not affect this
        // chain
        ...
        if (connectionPoint == BoneConnectionPoint.START) {
            connectionLocation =
                    mChains.get(existingChainNumber).getBone(existingBoneNumber).getStartLocation();
        ...
        {
            connectionLocation =
                    mChains.get(existingChainNumber).getBone(existingBoneNumber).getEndLocation();
        }
     ...
     * @param existingBoneNumber The index of the bone to connect the new chain to within the
     *     existing chain.
     * @param boneConnectionPoint Whether the new chain should connect to the START or END of the
     *     specified bone in the specified chain.
     */
    @Override
    public void connectChain(
            FabrikChain3D newChain,
            int existingChainNumber,
            int existingBoneNumber,
     ...
     *
     * <p>Chain numbers are zero indexed. If the specified chain does not exist in this structure
     ...
     *
     * @param fixedBaseMode Whether all chains should operate in fixed base mode (true) or not
     *     (false).
     */
