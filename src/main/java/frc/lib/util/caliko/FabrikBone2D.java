...
import frc.lib.util.caliko.utils.Mat4f;
 ...
 *
 * <p>A FabrikBone2D consists of a start location, an end location and a FabrikJoint2D which can
 * constrain the rotation of the bone with regard to either the previous bone in the same chain or
 * with regard to the direction of a bone in another chain which this bone is connected to.
 *
 * @author Al Lansley
...
public class FabrikBone2D implements FabrikBone<Vec2f, FabrikJoint2D> {
     ...
     *
     * <p>Each bone has a single FabrikJoint2D which controls the angle to which the bone is
     ...
     * constraint angles via the {@link #setClockwiseConstraintDegs(float)} and {@link
     * #setAnticlockwiseConstraintDegs(float)} methods.
     *
     ...
     * start, around which the bone may rotate (and which it may optionally be constrained). In this
     * way the single joint which can be considered to be at the start location of each bone
     * controls the allowable range of motion for that bone alone.
     */
     ...
     *
     * <p>The start location of a bone may only be set through a constructor or via an 'addBone' or
     * 'addConsecutiveBone' method provided by the {@link FabrikChain2D} class.
     */
     ...
     * identification of a bone, such as when used in a map or such.
     *
     * <p>Names exceeding 100 characters will be truncated.
     */
     ...
     *
     * <p>In the typical usage scenario of a FabrikBone2D the length of the bone remains constant.
     *
     * <p>The length may be set explicitly through a value provided to a constructor, or implicitly
     * when it is calculated as the distance between the {@link #mStartLocation} and {@link
     * mEndLocation} of a bone.
     *
     * <p>Attempting to set a bone length of less than zero, either explicitly or implicitly, will
     * result in an IllegalArgumentException or
     */
     ...
     *
     * <p>The default colour is white at full opacity.
     *
     * <p>This colour property does not have to be used, for example when using solved IK chains for
     * purposes other than drawing the solutions to screen.
     */
    ...
    /**
     * Constructor to create a new FabrikBone2D from a start and end location as provided by a pair
     * of Vec2fs.
     *
     ...
     *
     * <p>Instantiating a FabrikBone2D with the exact same start and end location, and hence a
     * length of zero, may result in undefined behaviour.
     *
    ...
    /**
     * Constructor to create a new FabrikBone2D from a start and end location as provided by a four
     * floats.
     *
     * <p>The {@link #mLength} property is calculated and set from the provided locations. All other
     * properties are set to their default values.
    ...
    /**
     * Constructor to create a new FabrikBone2D from a start location, a direction unit vector and a
     * length.
     *
     * <p>A normalised version of the direction unit vector is used to calculate the end location.
     *
     * <p>If the provided direction unit vector is zero then an IllegalArgumentException is thrown.
     * If the provided length argument is less than zero then an IllegalArgumentException is thrown.
     *
     * <p>Instantiating a FabrikBone3D with a length of precisely zero may result in undefined
     * behaviour.
     *
     * @param startLocation The start location of the bone in world-space.
    ...
    public FabrikBone2D(Vec2f startLocation, Vec2f directionUV, float length) {
        ...
        // Set the bone length via the setLength method rather than directly on the mLength property
        // so that validation is performed
        setLength(length);
    }

    /**
     * Constructor to create a new FabrikBone2D from a start location, a direction unit vector, a
     * length and a pair of constraint angles specified in degrees.
     *
     * <p>The clockwise and anticlockwise constraint angles can be considered to be relative to the
     * previous bone in the chain which this bone exists in UNLESS the bone is a basebone (i.e. the
     * first bone in a chain) in which case, the constraint angles can optionally be made relative
     * to either a world-space direction, the direction of the bone to which this bone may be
     * connected, or to a direction relative to the coordinate space of the bone to which this bone
     * may be connected.
     *
     * <p>If the direction unit vector argument is zero then an IllegalArgumentException is thrown.
     * If the length argument is less than zero then an IllegalArgumentException is thrown. If
     * either the clockwise or anticlockwise constraint angles are outside of the range 0.0f degrees
    ...
    /**
     * Constructor to create a new FabrikBone2D from a start location, a direction unit vector, a
     * length, a pair of constraint angles specified in degrees and a colour.
     *
     * <p>The clockwise and anticlockwise constraint angles can be considered to be relative to the
     * previous bone in the chain which this bone exists in UNLESS the bone is a basebone (i.e. the
     * first bone in a chain) in which case, the constraint angles can optionally be made relative
     ...
     * Copy constructor.
     *
     * <p>Takes a source FabrikBone2D object and copies all properties into the new FabrikBone2D by
     ...
     *
     * <p>Any colour component values outside the valid range of 0.0f to 1.0f inclusive are clamped
     * to that range.
     *
     ...
     *
     * <p>The valid range of constraint angle is 0.0f degrees to 180.0f degrees inclusive, angles
     ...
     *
     * <p>The valid range of constraint angle is 0.0f degrees to 180.0f degrees inclusive.
     *
     ...
     *
     * <p>The value set is clamped to be between 1.0f and 64.0f inclusive, if necessary.
     *
     ...
     *
     * @param coordSystem The coordinate system to use for any constraints applied to the joint of
     *     this bone.
     */
    public void setJointConstraintCoordinateSystem(
            FabrikJoint2D.ConstraintCoordinateSystem coordSystem) {
     ...
     *
     * <p>The colour and line-width are not included in this output, but can be queried separately
    ...
    public String toString() {
        ...
        sb.append(
                "Bone direction       : "
                        + Vec2f.getDirectionUV(mStartLocation, mEndLocation)
                        + Utils.NEW_LINE);
     ...
     *
     * <p>If the length argument is not greater then zero an IllegalArgumentException is thrown. If
     * the length argument is precisely zero then
     *
