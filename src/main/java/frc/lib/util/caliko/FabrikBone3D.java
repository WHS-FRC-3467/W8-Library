 ...
 *
 * <p>A FabrikBone3D consists of a start location, an end location and a FabrikJoint3D which can
 * constrain the rotation of the bone with regard to a previous bone in an IK chain either as a ball
 * joint or as a hinge joint constrained about a local or global axis.
 *
...
public class FabrikBone3D implements FabrikBone<Vec3f, FabrikJoint3D>, Serializable {
    ...
    /**
     * If this chain is connected to a bone in another chain, should this chain connect to the start
     * or the end of that bone?
     *
     * <p>The default is to connect to the end of the specified bone.
     *
     * <p>This property can be set via the {#link #setBoneConnectionPoint(BoneConnectionPoint)}
     * method, or when attaching this chain to another chain via the {@link
     * frc.lib.util.caliko.FabrikStructure3D#connectChain(FabrikChain3D, int, int,
     * BoneConnectionPoint)} method.
     */
     ...
     *
     * <p>Each bone has a single FabrikJoint3D which controls the angle to which the bone is
     * constrained with regard to the previous (i.e. earlier / closer to the base) bone in its
     * chain.
     *
     * <p>By default, a joint is not constrained (that is, it is free to rotate up to 180 degrees in
     * a clockwise or anticlockwise direction), however a joint may be constrained by specifying
     * constraint angles via the {@link #setBallJointConstraintDegs(float)}, {@link
     * #setHingeJointClockwiseConstraintDegs(float)}, and {@link
     * #setHingeJointAnticlockwiseConstraintDegs(float)} methods.
     *
     * <p>You might think that surely a bone has two joints, one at the beginning and one at the end
     * - however, consider an empty chain to which you add a single bone: It has a joint at its
     * start, around which the bone may rotate (and which it may optionally be constrained).
     *
     * <p>When a second bone is added to the chain, the joint at the start of this second bone
     * controls the rotational constraints with regard to the first ('base') bone, and so on.
     *
     * <p>During the forward (tip-to-base) pass of the FABRIK algorithm, the end effector is
     * unconstrained and snapped to the target. As we then work from tip-to-base each previous bone
     * is constrained by the joint in the outer bone until we reach the base, at which point, if we
     * have a fixed base location, then we snap the base bone start location to it, or if we do not
     * have a fixed base location we project the new start location along the reverse direction of
     * the bone by its length.
     *
     * <p>During the backward (base-to-tip) pass, each bone is constrained by the joint angles
     ...
     *
     * <p>The start location of a bone may only be set through a constructor or via an 'addBone'
     ...
     *
     * <p>The end location of a bone may only be set through a constructor or indirectly via an
     ...
     *
     * <p>It is not necessary to use this property, but it is provided to allow for easy
     * identification of a bone, such as when used in a map such as {@code Map<String,
     * FabrikBone3D>}.
     *
     * <p>The maximum allowable length of the name String is 100 characters - names exceeding this
    ...
    /**
     * The length of this bone from its start location to its end location. This is is calculated in
     * the constructor and remains constant for the lifetime of the object.
     */
    private float mLength;

    /**
     * The colour used to draw the bone as specified by a {@link Colour4f
     * frc.lib.util.caliko.utils.Colour4f.class} object.
     *
     * <p>The default colour to draw a bone is white at full opacity i.e. Colour4f(1.0f, 1.0f, 1.0f,
     * 1.0f).
     */
     ...
     *
     * <p>The default line width is 1.0f, which is the only value guaranteed to render correctly for
     ...
     * @see <a
     *     href="https://www.opengl.org/sdk/docs/man3/xhtml/glLineWidth.xml">glLineWidth(float)</a>
     */
     ...
     *
     * <p>The {@link #mLength} property is calculated and set from the provided locations. All other
     * properties are set to their default values.
     *
     * <p>Instantiating a FabrikBone3D with the exact same start and end location, and hence a
     * length of zero, will result in an IllegalArgumentException being thrown.
     *
    ...
    public FabrikBone3D(Vec3f startLocation, Vec3f endLocation) {
        ...
        // Set the length of the bone - if the length is not a positive value then an
     ...
     *
     * <p>This constructor is merely for convenience if you intend on working with named bones, and
     * internally calls the {@link #FabrikBone3D(Vec3f, Vec3f)} constructor.
     *
     * @param startLocation The start location of this bone.
     * @param endLocation The end location of this bone.
     ...
     *
     * <p>The end location of the bone is calculated as the start location plus the direction unit
     * vector multiplied by the length (which must be a positive value). All other properties are
     * set to their default values. *
     *
     * <p>If this constructor is provided with a direction unit vector of magnitude zero, or with a
    ...
    /**
     * Create a named FabrikBone3D from a start location, a direction unit vector, a bone length and
     * a name.
     *
     * <p>This constructor is merely for convenience if you intend on working with named bones, and
     * internally calls the {@link #FabrikBone3D(Vec3f, Vec3f, float)} constructor.
     *
     * <p>If the provided length argument is zero or if the direction is a zero vector then an
    ...
    /**
     * Create a new FabrikBone3D from a start location, a direction unit vector, a bone length and a
     * colour.
     *
     * <p>This constructor is merely for convenience if you intend on working with named bones, and
     * internally calls the {@link #FabrikBone3D(Vec3f, Vec3f, float)} constructor.
     ...
     * Copy constructor.
     *
     * <p>Takes a source FabrikBone3D object and copies all properties into the new FabrikBone3D by
    ...
    /**
     * Return the live (i.e. live calculated) length of this bone from its current start and end
     * locations.
     *
     * @return The 'live' calculated distance between the start and end locations of this bone.
     */
    public float liveLength() {
        return Vec3f.distanceBetween(mStartLocation, mEndLocation);
    }

    /**
     * Specify the bone connection point of this bone.
     *
     * <p>This connection point property controls whether, when THIS bone connects to another bone
     * in another chain, it does so at the start or the end of the bone we connect to.
     *
     * <p>The default is BoneConnectionPoint3D.END.
     *
     * @param bcp The bone connection point to use (BoneConnectionPoint3.START or
    ...
    /**
     * Return the bone connection point for THIS bone, which will be either
     * BoneConnectionPoint.START or BoneConnectionPoint.END.
     *
     * <p>This connection point property controls whether, when THIS bone connects to another bone
     * in another chain, it does so at the start or the end of the bone we connect to.
     *
    ...
    /**
     * Set the FabrikJoint3D of this bone to match the properties of the provided FabrikJoint3D
     * argument.
     *
    ...
    /**
     * Set the clockwise constraint angle of this bone's joint in degrees (0.0f to 180.0f
     * inclusive).
     *
     * <p>If a constraint angle outside of this range is provided, then an IllegalArgumentException
     * is thrown.
     *
    ...
    /**
     * Set the anticlockwise constraint angle of this bone's joint in degrees (0.0f to 180.0f
     * inclusive).
     *
     * <p>If a constraint angle outside of this range is provided, then an {@link
     * IllegalArgumentException} is thrown.
     *
     ...
     * IllegalArgumentException} is thrown.
     *
     * @param angleDegs The angle in degrees relative to the previous bone which this bone is
     *     constrained to.
     */
    public void setBallJointConstraintDegs(float angleDegs) {
        if (angleDegs < 0.0f || angleDegs > 180.0f) {
            throw new IllegalArgumentException(
                    "Rotor constraints for ball joints must be in the range 0.0f to 180.0f degrees"
                            + " inclusive.");
        }
     ...
     *
     * <p>If the opposite (i.e. end to start) location is required then you can simply negate the
     * provided direction.
     *
    ...
    /**
     * Get the global pitch of this bone with regard to the X-Axis. Pitch returned is in the range
     * -179.9f to 180.0f.
     *
    ...
    /**
     * Get the global yaw of this bone with regard to the Y-Axis. Yaw returned is in the range
     * -179.9f to 180.0f.
     *
     ...
     *
     * <p>If the provided parameter is outside the valid range of 1.0f to 64.0f inclusive then an
     * IllegalArgumentException is thrown.
     *
     ...
     *
     * <p>If the bone has not been specifically named through a constructor or by using the {@link
     * #setName(String)} method, then the name will be the default of "UnnamedFabrikBone3D".
     *
     * @return String
     */
     ...
     *
     * <p>This method validates the length argument to ensure that it is greater than zero.
     *
     * <p>If the length argument is not a positive value then an {@link IllegalArgumentException} is
     * thrown.
     *
