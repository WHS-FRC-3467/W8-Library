...
/**
 * A FabrikStructure2D contains one or more FabrikChain2D objects, which we can solve using the
 * FABRIK (Forward And Backward Reaching Inverse Kinematics) algorithm for specified target
 * locations.
 *
 * <p>The FabrikStructure2D class is merely a convenient holder for an ArrayList of FabrikChain2D
 * objects which allows multiple chains to have their target location updated, as well as drawing
 * the multiple chains attached to the FabrikStructure2D object, using a single method call.
 *
 * <p>Chains in a structure may be connected to other chains in the same structure in a variety of
 * ways, if desired.
 *
 * @author Al Lansley
...
public class FabrikStructure2D implements FabrikStructure<FabrikChain2D, Vec2f>, Serializable {
    ...
    /**
     * Property to indicate if the first chain (chain zero) in this structure has its basebone fixed
     * in place or not.
     */
     ...
     * Naming constructor.
     *
     * <p>Names lengths are limited to 100 characters and are truncated if necessary.
     *
     * @param name The name you wish to call the structure.
     */
    public FabrikStructure2D(String name) {
        setName(name);
    }
     ...
     *
     * <p>All chains in this structure are solved for the given target location EXCEPT those which
     * have embedded targets enabled, which are solved for the target location embedded in the
     * chain.
     *
     * <p>After this method has been executed, the configuration of all IK chains attached to this
     * structure will have been updated.
     *
     * @param newTargetLocation The location of the target for which we will attempt to solve all
     *     chains attached to this structure.
     */
    @Override
    public void solveForTarget(Vec2f newTargetLocation) {
        ...
        for (int loop = 0; loop < numChains; ++loop) {
            ...
            // If this chain is not connected to another chain and the basebone constraint type of
            // this chain is not global absolute
            // then we must update the basebone constraint UV for LOCAL_RELATIVE and the basebone
            // relative constraint UV for LOCAL_ABSOLUTE connection types.
            // Note: For NONE or GLOBAL_ABSOLUTE we don't need to update anything before calling
            // updateTarget().
            if (hostChainNumber != -1
                    && constraintType != BaseboneConstraintType2D.GLOBAL_ABSOLUTE) {
                // Get the bone which this chain is connected to in the 'host' chain
                FabrikBone2D hostBone =
                        mChains.get(hostChainNumber)
                                .getBone(mChains.get(loop).getConnectedBoneNumber());

                // If we're connecting this chain to the start location of the bone in the 'host'
                // chain...
                if (thisChain.getBoneConnectionPoint() == BoneConnectionPoint.START) {
                    // ...set the base location of this bone to be the start location of the bone
                    // it's connected to.
                    thisChain.setBaseLocation(hostBone.getStartLocation());
                } else // If the bone connection point is BoneConnectionPoint.END...
                {
                    // ...set the base location of the chain to be the end location of the bone
                    // we're connecting to.
                ...
                if (constraintType == BaseboneConstraintType2D.LOCAL_RELATIVE) {
                    // ...then set the basebone constraint UV to be the direction of the bone we're
                    // connected to.
                    mChains.get(loop).setBaseboneConstraintUV(hostBoneUV);
                } else if (constraintType == BaseboneConstraintType2D.LOCAL_ABSOLUTE) {
                    // Note: LOCAL_ABSOLUTE directions are directions which are in the local
                    // coordinate system of the host bone.
                    // For example, if the baseboneConstraintUV is Vec2f(-1.0f, 0.0f) [i.e. left],
                    // then the baseboneConnectionConstraintUV
                    ...
                    // ...then apply that same rotation to this chain's basebone constraint UV to
                    // get the relative constraint UV...
                    Vec2f relativeConstraintUV =
                            Vec2f.rotateDegs(thisChain.getBaseboneConstraintUV(), angleDegs);

                    // ...which we then update.
                    thisChain.setBaseboneRelativeConstraintUV(relativeConstraintUV);
                }

                // NOTE: If the basebone constraint type is NONE then we don't do anything with the
                // basebone constraint of the connected chain.
     ...
     *
     * <p>Adding a chain using this method adds the chain to the structure, but does not connect it
     * to any existing chain in the structure. However, all chains in a structure share the same
     * target, and all chains in the structure can be solved for the target location via a single
     * call to updateTarget on this structure.
     *
    ...
    /**
     * Add a chain to this structure which is connected to a specific bone of an existing chain in
     * this structure.
     *
     * <p>When connecting a chain to an existing chain in a structure, the multiple chains will all
     * share the same target location / end effector.
     *
     * <p>The location of the bones in the chain to be connected should be specified relative to the
     * origin (0.0f, 0.0f), not relative to the current location of connection point in the chain
     * being connected to. On connection, all bones are translated (that is, moved horizontally and
     * vertically) so that they're positioned relative to the end location of the specified bone in
     * the specified chain.
     *
     * <p>Both chains within the structure and bones within a chain are zero. If the chainNumber or
     * boneNumber specified do not exist then an IllegalArgumentException is thrown.
     *
     ...
     * @param boneNumber The zero indexed number of the bone within the specified chain to connect
     *     the provided chain to.
     */
    @Override
    public void connectChain(FabrikChain2D chain, int chainNumber, int boneNumber) {
        ...
        // Get the connection point so we know to connect at the start or end location of the bone
        // we're connecting to
        ...
        } else // If it's BoneConnectionPoint.END then we set the connection point to be the end
        // location of the bone we're connecting to
        {
        ...
        // When we have a chain connected to a another 'host' chain, the chain is which is
        // connecting in
        // MUST have a fixed base, even though that means the base location is 'fixed' to the
        // connection
     ...
     * @param chainNumber The zero-indexed number of the pre-existing chain in this structure that
     *     the new chain will attach to.
     * @param boneNumber The zero-indexed number of bone in the pre-existing chain in this structure
     *     that the new chain will attach to.
     * @param boneConnectionPoint Whether to attach the new chain to the start or end point of the
     *     bone we are connecting to.
     */
    @Override
    public void connectChain(
        ...
        // Set the bone connection point so we'll connect to the start or the end point of the
        // specified connection bone
     ...
     *
     * <p>Note: Chains are zero-indexed.
     *
     ...
     *
     * <p>If disabling fixed-base mode and the first chain has a BaseBoneConstraintType of
     * GLOBAL_ABSOLUTE then a RuntimeException will be thrown as this these options are mutually
     * exclusive.
     *
     * @param fixedBaseMode Whether we want this structure to operate in fixed-base mode (true) or
     *     not (false).
     */
    public void setFixedBaseMode(boolean fixedBaseMode) {
        // Update our flag and set the fixed base mode on the first (i.e. 0th) chain in this
        // structure.
     ...
     *
     * <p>If further details on a specific chain are required, then you should get and print each
     * chain individually.
     */
    @Override
