...
public class ArmJointSim extends LinearSystemSim<N2, N1, N2> {
    ...
    private @Setter Optional<AngularVelocity> topVelocity;
    private @Setter Optional<Double> bottomAngle;
    ...
    public ArmJointSim(
        ...
        topVelocity = Optional.empty();
        bottomAngle = Optional.empty();
    }
    ...
    public final void setState(double angleRadians, double velocityRadPerSec) {
        setState(
                VecBuilder.fill(
                        MathUtil.clamp(angleRadians, m_minAngle, m_maxAngle), velocityRadPerSec));
    }
                            ...
                            if (m_simulateGravity) {
                                ...
                                double alphaGrav =
                                        3.0
                                                ...
                                                * -9.8
                                                * Math.cos(
                                                        bottomAngle.isPresent()
                                                                ? x.get(0, 0) + bottomAngle.get()
                                                                : x.get(0, 0))
                                                / m_armLenMeters;

                                if (topVelocity.isPresent()) {

                                    alphaGrav -= (((topVelocity.get().in(RadiansPerSecond) / 6)));
                                }

                                Logger.recordOutput("alphaGrav" + this.hashCode(), alphaGrav);
